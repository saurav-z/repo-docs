...
**Simplify your data streaming with FastStream, a Python framework built for building and integrating message-driven microservices.**
...
FastStream simplifies event stream integration for your services, making it easier than ever to build message-driven microservices. Designed for ease of use and scalability, here's what makes FastStream stand out:

*   **Unified API for Multiple Brokers**: Seamlessly work with Kafka, RabbitMQ, NATS, and Redis using a consistent API.
*   **Pydantic Validation**: Easily validate incoming messages with Pydantic's robust data validation capabilities.
*   **Automated AsyncAPI Documentation**: Generate AsyncAPI documentation automatically, streamlining service integration and communication.
...
*   **Seamless Integrations**: Compatible with any HTTP framework, with a dedicated FastAPI plugin for streamlined integration.

For more details, please check out the [FastStream Documentation](https://faststream.ag2.ai/latest/).
...
FastStream works on Linux, macOS, Windows, and most Unix-style operating systems. It uses Pydantic V2, written in Rust, for optimal performance. However, it can work with Pydantic V1 if necessary.

---

## Quick Start

FastStream uses function decorators `@broker.subscriber` and `@broker.publisher` to simplify consuming, producing, and decoding/encoding messages.
...
FastStream also integrates with Pydantic's `BaseModel` for easy data validation and type enforcement:
...
## Testing with FastStream

Easily test your service with the `TestBroker` context manager:
...
1.  Install the FastStream CLI: `pip install "faststream[cli]"`
2.  Run your application: `faststream run basic:app`

Use `--reload` for hot reloading and `--workers` for multiprocessing. For details on the CLI, visit the [CLI documentation](https://faststream.ag2.ai/latest/getting-started/cli/).
...
FastStream (via [**FastDepends**](https://lancetnik.github.io/FastDepends/)) has a dependency management system similar to `pytest fixtures` and `FastAPI Depends` at the same time. Function arguments declare which dependencies you want are needed, and a special decorator delivers them from the global Context object.
...
[FastStream](https://github.com/ag2ai/faststream) is an open-source project.
...
Thanks to the amazing people who contribute to FastStream:
