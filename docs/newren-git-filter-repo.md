# Git Filter-Repo: Rewrite Git History with Ease

**Tired of slow, error-prone history rewriting?**  Git filter-repo is the recommended tool for efficiently and safely rewriting your Git repository's history.  [Visit the original repository on GitHub](https://github.com/newren/git-filter-repo)

## Key Features

*   **Superior Performance:** Outperforms `git filter-branch` by orders of magnitude, especially for complex rewrites.
*   **Enhanced Safety:** Designed to avoid the pitfalls and data corruption risks associated with `git filter-branch`.
*   **Comprehensive Functionality:** Offers a wider range of history rewriting capabilities beyond what other tools provide.
*   **User-Friendly:** Easier to use than `git filter-branch`, with clear command syntax and helpful examples.
*   **Extensible:** Provides a library for creating custom history rewriting tools tailored to specialized needs.
*   **Built-in Analysis:** Offers initial repository analysis to help users get started on their desired changes.
*   **Path Renaming:** Easily rename files and directories, including making subdirectories the root.
*   **Automated Cleanup:** Automatically removes old objects and repacks the repository after filtering.
*   **Commit Message Rewriting:** Rewrites commit messages to reflect the new commit IDs.
*   **Empty Commit Handling:** Correctly prunes commits that become empty due to filtering.
...
Installation is simple: just place the single-file python script `git-filter-repo` into your `$PATH`.

For more complex setups, such as those involving custom python executables or documentation, refer to [INSTALL.md](INSTALL.md).

## How do I use it?

*   **Comprehensive Documentation:** Explore the [user manual](https://htmlpreview.github.io/?https://github.com/newren/git-filter-repo/blob/docs/html/git-filter-repo.html) for detailed guidance.
*   **Alternative Documentation:** For alternative formatting, see sites like [mankier.com](https://www.mankier.com/1/git-filter-repo).
...
Git filter-repo offers significant advantages over alternative tools like `filter-branch` and BFG Repo Cleaner.

### filter-branch

*   **Performance Issues:** `filter-branch` is notoriously slow, especially for large repositories.
*   **Safety Concerns:** Prone to data corruption and unexpected behavior.
*   **Complex Usage:** Difficult to use for non-trivial rewriting tasks.
*   **Deprecated Recommendation:** The Git project recommends *against* using `filter-branch`.
*   **Alternative:** Consider [filter-lamely](contrib/filter-repo-demos/filter-lamely), a reimplementation of filter-branch based on filter-repo.
*   [Cheat sheet](Documentation/converting-from-filter-branch.md#cheat-sheet-conversion-of-examples-from-the-filter-branch-manpage)

### BFG Repo Cleaner

*   **Limited Functionality:** Restricted to a few specific rewriting use cases.
*   **Architectural Limitations:** Difficult to extend for new rewrite types.
*   **Potential Shortcomings:** May exhibit bugs even within its intended use cases.
*   **Alternative:** Consider [bfg-ish](contrib/filter-repo-demos/bfg-ish), a reimplementation of bfg based on filter-repo.
*   [Cheat sheet](Documentation/converting-from-bfg-repo-cleaner.md#cheat-sheet-conversion-of-examples-from-bfg)
...
Git filter-repo was created to overcome the limitations of existing tools and provide a more powerful and reliable solution.  The design incorporates these key aspects:
...
The filter-repo community follows the [git Code of Conduct](https://git.kernel.org/pub/scm/git/git.git/tree/CODE_OF_CONDUCT.md).

## Upstream Improvements

The development of filter-repo has driven numerous improvements to core Git commands.  These improvements enhance the performance, reliability, and functionality of Git for everyone. The following commits have been added to the core git command set due to the development of git filter-repo:
