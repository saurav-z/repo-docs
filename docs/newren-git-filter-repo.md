...
Tired of slow and error-prone git history rewriting? **git-filter-repo** is the recommended alternative to `git filter-branch`, offering superior performance, flexibility, and safety for complex Git history manipulations. Check out the [original repo](https://github.com/newren/git-filter-repo).

**Key Features:**

*   **Blazing Fast:** Significantly faster than `git filter-branch`, especially for large repositories.
*   **Comprehensive:** Offers a wide range of rewriting capabilities beyond simple filtering.
*   **Safe & Reliable:** Designed to avoid common pitfalls and data corruption issues of `git filter-branch`.
*   **Intuitive:** Simplifies complex rewriting tasks with easy-to-use commands.
*   **Extensible:** Provides a library for creating custom history rewriting tools.
*   **Path-Based Filtering**: Easily keep or remove specific paths and rename them.
*   **Commit Message Rewriting:** Automatically updates commit messages to reflect new commit IDs.
*   **Smart Pruning**: Efficiently removes empty and degenerate commits resulting from filtering.
*   **Fresh Clone Protection:** Prompts users to use a fresh clone for safety unless overridden with --force.
...
The core functionality of `git-filter-repo` resides in a single Python script, making installation straightforward: place the `git-filter-repo` file into your system's `$PATH`.

Refer to [INSTALL.md](INSTALL.md) for more complex installation scenarios. This is generally only necessary if:

*   You require specific Python versions.
*   You wish to install documentation or run the examples in `contrib`.
*   You want to create custom Python filtering scripts using the library.
...
*   Explore alternative formatted manuals on external sites (e.g., [mankier.com](https://www.mankier.com/1/git-filter-repo)).

For learning through examples:

*   Use the [cheat sheet for converting filter-branch commands](Documentation/converting-from-filter-branch.md#cheat-sheet-conversion-of-examples-from-the-filter-branch-manpage).
*   Use the [cheat sheet for converting BFG Repo Cleaner commands](Documentation/converting-from-bfg-repo-cleaner.md#cheat-sheet-conversion-of-examples-from-bfg).
*   See the [simple example](#simple-example-with-comparisons) below.
*   Check the [examples section](https://htmlpreview.github.io/?https://github.com/newren/git-filter-repo/blob/docs/html/git-filter-repo.html#EXAMPLES) in the user manual.
*   Review [example filterings based on user-filed issues](Documentation/examples-from-user-filed-issues.md).

You might also find the [Frequently Answered Questions](Documentation/FAQ.md) helpful.

## Why filter-repo instead of other alternatives?

More details are available in a [Git Rev News article on
filter-repo](https://git.github.io/rev_news/2019/08/21/edition-54/#an-introduction-to-git-filter-repo--written-by-elijah-newren), but the main competitors are:

### filter-branch

*   `filter-branch` is extremely slow for non-trivial repositories (multiple orders of magnitude slower).
*   `filter-branch` has numerous pitfalls that can corrupt your rewrite.
*   `filter-branch` is difficult to use for any rewrite which is even slightly non-trivial.
*   The Git project recommends that you stop using `filter-branch`.
*   Users may be interested in `filter-lamely` (a.k.a. `filter-branch-ish`), a reimplementation of `filter-branch` based on filter-repo which is more performant.
*   A [cheat sheet](Documentation/converting-from-filter-branch.md#cheat-sheet-conversion-of-examples-from-the-filter-branch-manpage) is available to help you convert commands from `filter-branch`.

### BFG Repo Cleaner

*   BFG is limited to a few types of rewrites.
*   Its architecture is not amenable to handling more types of rewrites.
*   Fans of BFG may be interested in `bfg-ish`, a reimplementation of BFG based on filter-repo which includes several new features and bugfixes.
*   A [cheat sheet](Documentation/converting-from-bfg-repo-cleaner.md#cheat-sheet-conversion-of-examples-from-bfg) is available to help you convert commands from BFG Repo Cleaner.
...
The tool offers some of the most important features.
...
`git-filter-repo` has driven significant improvements to core Git commands, including `fast-export` and `fast-import`.