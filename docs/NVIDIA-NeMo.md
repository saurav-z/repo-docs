...
NVIDIA NeMo is a cloud-native framework for building, training, and deploying state-of-the-art generative AI models, including LLMs, MMs, ASR, TTS, and CV models. ([See the original repository](https://github.com/NVIDIA/NeMo)).
...
*   **Scalable Training:** Designed for efficient training on thousands of GPUs, leveraging techniques like Tensor Parallelism, Pipeline Parallelism, and Mixed Precision Training.
*   **Modular Architecture:** NeMo 2.0 utilizes Python-based configuration and PyTorch Lightning's modular abstractions for enhanced flexibility and customization.
*   **Model Alignment & Customization:** Supports state-of-the-art alignment methods like SteerLM and Direct Preference Optimization (DPO), as well as parameter-efficient fine-tuning (PEFT) techniques.
*   **Deployment & Optimization:** Enables deployment and optimization with NVIDIA NeMo Microservices and Riva for speech AI.
*   **Pre-trained Models & Resources:** Offers readily available pre-trained models on Hugging Face Hub and NVIDIA NGC, along with extensive tutorials, example scripts, and playbooks.
*   **Cosmos Support:** Supports training and customizing the NVIDIA Cosmos collection of world foundation models.
*   **Hugging Face Model Support**: Integration with Hugging Face models.

## Latest News

*   **Pretrain and finetune Hugging Face models via AutoModel**: NeMo Framework's latest feature AutoModel enables broad support for Hugging Face models, with 25.04 focusing on `AutoModelForCausalLM` in the Text Generation category, and `AutoModelForImageTextToText` in the Image-Text-to-Text category.
...
*   **NeMo Framework 2.0:** Released NeMo 2.0, which prioritizes modularity and ease-of-use.
*   **New Cosmos World Foundation Models Support**: Advancing Physical AI with NVIDIA Cosmos World Foundation Model Platform and Accelerate Custom Video Foundation Model Pipelines with New NVIDIA NeMo Framework Capabilities.
*   **Large Language Models and Multimodal Models**: State-of-the-Art Multimodal Generative AI Model Development with NVIDIA NeMo, New Llama 3.1 Support, Accelerate your Generative AI Distributed Training Workloads with the NVIDIA NeMo Framework on Amazon EKS, NVIDIA NeMo Accelerates LLM Innovation with Hybrid State Space Model Support, NVIDIA releases 340B base, instruct, and reward models pretrained on a total of 9T tokens, NVIDIA sets new generative AI performance and scale records in MLPerf Training v4.0, Accelerate your generative AI journey with NVIDIA NeMo Framework on GKE
*   **Speech Recognition**: Accelerating Leaderboard-Topping ASR Models 10x with NVIDIA NeMo, New Standard for Speech Recognition and Translation from the NVIDIA NeMo Canary Model, Pushing the Boundaries of Speech Recognition with NVIDIA NeMo Parakeet ASR Models, Turbocharge ASR Accuracy and Speed with NVIDIA NeMo Parakeet-TDT

## Introduction

NVIDIA NeMo is a versatile, cloud-native framework designed for researchers and developers working with generative AI. It enables efficient creation, customization, and deployment of new generative AI models by leveraging existing code and pre-trained model checkpoints.
