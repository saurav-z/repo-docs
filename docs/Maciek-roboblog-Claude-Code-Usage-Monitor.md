...
**Stay in control of your Claude AI usage with the Claude Code Usage Monitor!**  Monitor your token consumption in real-time, get intelligent session limit predictions, and analyze your costs with this powerful terminal tool.  [View the Original Repo](https://github.com/Maciek-roboblog/Claude-Code-Usage-Monitor)
...
Get real-time insights into your Claude AI usage with this comprehensive terminal monitor.  Track token consumption, analyze burn rates, and proactively manage costs.  The Claude Code Usage Monitor features a beautiful Rich UI with advanced analytics and ML-powered predictions to help you optimize your Claude AI sessions.
...
*   **🔮 ML-based Predictions:**  Intelligent session limit detection using P90 percentile calculations.
*   **🔄 Real-time Monitoring:**  Configurable refresh rates (0.1-20 Hz) with dynamic display updates.
*   **📊 Advanced Rich UI:**  Visually appealing, color-coded progress bars, tables, and layouts (WCAG-compliant).
*   **🤖 Smart Auto-Detection:**  Automatic plan switching and custom limit discovery.
*   **📋 Enhanced Plan Support:** Supports Pro, Max5, Max20, and Custom plans.
*   **⚠️ Advanced Warning System:**  Multi-level alerts with cost and time projections.
*   **🎨 Intelligent Theming:** Scientific color schemes with automated terminal background detection.
*   **📈 Cost Analytics:** Model-specific pricing with cache token calculations.
*   **📝 Comprehensive Logging:**  Optional file logging with customizable levels.
*   **🧪 Extensive Testing:**  100+ test cases for full code coverage.
...
*   **Custom Token Limits:**  `claude-monitor --plan custom --custom-limit-tokens <tokens>`
*   **Reset Time:** `claude-monitor --reset-hour <hour (0-23)>`
*   **View Type:** `claude-monitor --view [realtime|daily|monthly]`
*   **Refresh Rate:** `claude-monitor --refresh-rate <seconds>` and `--refresh-per-second <Hz>`
*   **Timezone:**  `claude-monitor --timezone <timezone>` (e.g., `America/New_York`, `UTC`)
*   **Theme:** `claude-monitor --theme [light|dark|classic|auto]`
*   **Logging:**  `claude-monitor --log-file <path>` and `--log-level [DEBUG|INFO|WARNING|ERROR|CRITICAL]`
