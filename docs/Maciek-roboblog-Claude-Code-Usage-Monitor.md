...
**Tired of guessing your Claude AI token usage?** Get precise control with the Claude Code Usage Monitor, a powerful terminal tool that provides real-time token tracking, insightful analytics, and intelligent session limit predictions. [Check it out on GitHub!](https://github.com/Maciek-roboblog/Claude-Code-Usage-Monitor)
...
Stay in control of your Claude AI usage with our robust monitor, featuring real-time updates, advanced analytics, and intelligent session management.
...
*   📊 **Real-time Monitoring:** Track token consumption, burn rate, and cost analysis in real-time with configurable refresh rates.
*   🔮 **AI-Powered Predictions:** Utilize machine learning for intelligent session limit detection and personalized usage predictions.
*   🎨 **Rich Terminal UI:** Experience a visually appealing interface with color-coded progress bars, tables, and WCAG-compliant contrast.
*   🤖 **Smart Auto-Detection:** Automatically switches plans and customizes limits, optimizing your usage based on your patterns.
*   ✅ **Advanced Plan Support:** Supports all major plans, including Pro, Max5, and Max20, with custom limits based on your needs.
*   ⚠️ **Advanced Warning System:** Receive multi-level alerts with cost and time predictions, helping you stay within your limits.

### v3.0.0 Major Update: Complete Architecture Rewrite

*   🔮 ML-based predictions for P90 percentile calculations and intelligent session limit detection
*   🔄 Real-time monitoring with configurable refresh rates (0.1-20 Hz) and intelligent display updates.
*   📊 Advanced Rich UI with color-coded progress bars, tables, layouts, and WCAG-compliant contrast.
*   🤖 Smart auto-detection for automatic plan switching with custom limit discovery.
*   📋 Enhanced plan support, with Pro (19k), Max5 (88k), Max20 (220k), and Custom (P90-based) limits.
*   ⚠️ Advanced warning system for multi-level alerts with cost and time predictions.
*   💼 Professional Architecture with modular design and Single Responsibility Principle (SRP) compliance.
*   🎨 Intelligent theming with scientific color schemes and automatic terminal background detection.
*   ⏰ Advanced scheduling for auto-detected system timezone and time format preferences.
*   📈 Cost analytics with model-specific pricing and cache token calculations.
*   🔧 Pydantic validation for type-safe configuration with automatic validation.
*   📝 Comprehensive logging with optional file logging and configurable levels.
*   🧪 Extensive testing with 100+ test cases with full coverage.
*   🎯 Error reporting with optional Sentry integration for production monitoring.
*   ⚡ Performance optimization with advanced caching and efficient data processing.

### Default Custom Plan

The **Custom plan** is the default option, specifically designed for 5-hour Claude Code sessions. This plan monitors three crucial metrics:

*   **Token Usage:** Tracks your token consumption
*   **Message Usage:** Monitors your message count
...
The new version features a complete rewrite with modular architecture following the Single Responsibility Principle (SRP):
...
4.  **Notification**: Displays a clear message about the change
...
4.  **Sets Limits**: Uses discovered maximum as a new limit
...
**Scenario**: You just want to see the current status without configuration.
...
*   **Realtime**: Live monitoring of the current session and burn rate
...
1.  **Terminal Setup**
    ...
    *   Enable color support for better visual feedback (check the `COLORTERM` environment variable)
    *   Consider a dedicated terminal window for monitoring
    *   Use terminals with truecolor support for the best theme experience
...
5.  **4:00 PM**: Light tasks, prepare for the evening session
...
If you don't have the `venv` module available:
...
python3 -m venv venv
# Or if using the virtualenv package:
# virtualenv venv
...
If the `claude-monitor` command is not found after pip installation:
