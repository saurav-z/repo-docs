# 🗂️ LlamaIndex: Build Powerful LLM Applications with Your Data

**LlamaIndex empowers you to build context-aware applications using your own data, unlocking the full potential of Large Language Models (LLMs).**  Learn more on the original repo: [https://github.com/run-llama/llama_index](https://github.com/run-llama/llama_index)

[![PyPI - Downloads](https://img.shields.io/pypi/dm/llama-index)](https://pypi.org/project/llama-index/)
[![Build](https://github.com/run-llama/llama_index/actions/workflows/build_package.yml/badge.svg)](https://github.com/run-llama/llama_index/actions/workflows/build_package.yml)
[![GitHub contributors](https://img.shields.io/github/contributors/jerryjliu/llama_index)](https://github.com/jerryjliu/llama_index/graphs/contributors)
[![Discord](https://img.shields.io/discord/1059199217496772688)](https://discord.gg/dGcwcsnxhU)
[![Twitter](https://img.shields.io/twitter/follow/llama_index)](https://x.com/llama_index)
[![Reddit](https://img.shields.io/reddit/subreddit-subscribers/LlamaIndex?style=plastic&logo=reddit&label=r%2FLlamaIndex&labelColor=white)](https://www.reddit.com/r/LlamaIndex/)
[![Ask AI](https://img.shields.io/badge/Phorm-Ask_AI-%23F2777A.svg?&logo=data:image/svg+xml;base64,)](https://www.phorm.ai/query?projectId=c5863b56-6703-4a5d-87b6-7e6031bf16b6)

## Key Features

*   **Data Connectors:** Ingest data from various sources (APIs, PDFs, databases, etc.).
*   **Data Structuring:** Organize your data using indices and graphs for LLM compatibility.
*   **Advanced Retrieval & Querying:** Get context-aware outputs by querying your data with natural language prompts.
*   **Customization:**  Offers both high-level APIs for ease of use and lower-level APIs for advanced customization of every module.
*   **Seamless Integrations:** Integrates with popular frameworks like LangChain, Flask, and more.

## Getting Started

LlamaIndex offers two primary ways to get started:

1.  **Starter Package (`llama-index`):**  Includes core LlamaIndex and a selection of integrations.
2.  **Customized Installation (`llama-index-core`):** Install the core and choose your integrations from [LlamaHub](https://llamahub.ai/).  LlamaHub features over 300 integration packages for your preferred LLM, embedding, and vector store providers.

### Installation

```bash
# Example: Install the starter package
pip install llama-index
```

### Basic Usage Example

```python
import os

os.environ["OPENAI_API_KEY"] = "YOUR_OPENAI_API_KEY"

from llama_index.core import VectorStoreIndex, SimpleDirectoryReader

# Load data from a directory
documents = SimpleDirectoryReader("YOUR_DATA_DIRECTORY").load_data()

# Create a vector index
index = VectorStoreIndex.from_documents(documents)

# Query the index
query_engine = index.as_query_engine()
response = query_engine.query("YOUR_QUESTION")

print(response)
```

## Important Links

*   [Documentation](https://docs.llamaindex.ai/en/stable/)
*   [LlamaIndex.TS (Typescript/Javascript)](https://github.com/run-llama/LlamaIndexTS)
*   [X (formerly Twitter)](https://x.com/llama_index)
*   [LinkedIn](https://www.linkedin.com/company/llamaindex/)
*   [Reddit](https://www.reddit.com/r/LlamaIndex/)
*   [Discord](https://discord.gg/dGcwcsnxhU)

## Ecosystem

*   [LlamaHub (Community Data Loaders)](https://llamahub.ai)
*   [LlamaLab (AGI Projects)](https://github.com/run-llama/llama-lab)

## Contributing

We welcome contributions! Check out our [Contribution Guide](CONTRIBUTING.md) for details.

## Citation

```
@software{Liu_LlamaIndex_2022,
author = {Liu, Jerry},
doi = {10.5281/zenodo.1234},
month = {11},
title = {{LlamaIndex}},
url = {https://github.com/jerryjliu/llama_index},
year = {2022}
}