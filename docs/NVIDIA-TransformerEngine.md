...
Transformer Engine is a powerful library designed to boost the performance of Transformer models on NVIDIA GPUs, offering significant speedups and reduced memory usage through FP8 precision.  [See the original repository](https://github.com/NVIDIA/TransformerEngine).

**Key Features:**

*   **FP8 Support:**  Leverage 8-bit floating point (FP8) precision on Hopper, Ada, and Blackwell GPUs for faster training and inference.
*   **Optimized Building Blocks:**  Utilize highly optimized modules tailored for popular Transformer architectures.
*   **Automatic Mixed Precision:**  An intuitive API that seamlessly integrates with your existing framework code.
*   **Framework Agnostic C++ API:** Provides a C++ API that can be integrated with other deep learning libraries, enabling FP8 support for Transformers.
*   **FP8 Support:**  Supports all precisions (FP16, BF16) on NVIDIA Ampere and newer GPU architectures.
...
Transformer Engine (TE) is a library that accelerates Transformer models on NVIDIA GPUs, including the use of 8-bit floating point (FP8) precision. This results in better performance with lower memory utilization in both training and inference.
...
*   **System Requirements:**
    ...
    *   **Software:** CUDA 12.1+ (Hopper/Ada/Ampere), 12.8+ (Blackwell) with compatible NVIDIA drivers, cuDNN 9.3+, GCC 9+ or Clang 10+ with C++17 support, Python 3.12 recommended
    *   **Source Build Requirements:** CMake 3.18+, Ninja, Git 2.17+, pybind11 2.6.0+
    *   **Notes:** FP8 features require Compute Capability 8.9+ (Ada/Hopper/Blackwell)

*   **Installation Methods:**
    *   **Docker (Recommended):** Use pre-built images from the [NVIDIA GPU Cloud (NGC) Catalog](https://catalog.ngc.nvidia.com/orgs/nvidia/containers/pytorch).
        ```bash
...
*   **Environment Variables:**
    ...
    *   `MAX_JOBS`: Limit number of parallel build jobs
    *   `NVTE_BUILD_THREADS_PER_JOB`: Control threads per build job

*   **Compiling with FlashAttention:**
    *   Verify FlashAttention version with: `NVTE_DEBUG=1 NVTE_DEBUG_LEVEL=1 python your_script.py`
    *   Address potential out-of-memory errors during FlashAttention-2 compilation by setting `MAX_JOBS=1`.

**Troubleshooting**

1.  **ABI Compatibility Issues:** Ensure PyTorch and Transformer Engine are built with the same C++ ABI setting.
2.  **Missing Headers or Libraries:** Install development packages or set environment variables (e.g., `CUDA_PATH`, `CUDNN_PATH`). Set `CXX` if CMake can't find a C++ compiler.
3.  **Build Resource Issues:** Limit parallel builds using `MAX_JOBS=1` and `NVTE_BUILD_THREADS_PER_JOB=1`.
4.  **Verbose Build Logging:** Use `pip install -v -v -v --no-build-isolation .` inside the `transformer_engine` directory.

**Breaking Changes**

*   **v1.7: Padding Mask Definition for PyTorch:**  Padding mask definition changed to `True` meaning masking out the corresponding position, `False` meaning including that position in attention calculation.

**FP8 Convergence**

FP8 has been extensively tested and demonstrates **no significant difference** in convergence compared to BF16.  See the table below for examples:
