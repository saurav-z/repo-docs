...
# Transformer Engine: Accelerate Transformer Models with FP8

**Supercharge your Transformer model training and inference with NVIDIA Transformer Engine, achieving significant performance gains and reduced memory usage using cutting-edge FP8 precision.**  [View the original repository on GitHub](https://github.com/NVIDIA/TransformerEngine).

**Key Features:**

*   **FP8 Precision:** Support for 8-bit floating point (FP8) on NVIDIA Hopper, Ada, and Blackwell GPUs for faster training and inference.
*   **Optimized Kernels:** Includes fused kernels and other optimizations tailored for Transformer models.
*   **Framework Agnostic C++ API:** Integrate FP8 support into other deep learning libraries.
*   **Easy-to-Use Modules:** Python modules for building Transformer layers with FP8 support.
*   **Mixed Precision Support:** Optimizations for FP16, and BF16 on Ampere and later GPU architectures.
...
*   **[03/2025]** [Measure and Improve AI Workload Performance with NVIDIA DGX Cloud Benchmarking](https://developer.nvidia.com/blog/measure-and-improve-ai-workload-performance-with-nvidia-dgx-cloud-benchmarking/)
    ![FP8 vs. BF16 comparison](docs/examples/comparison-fp8-bf16-training-nvidia-dgx-cloud-benchmarking-performance-explorer.jpg "FP8 vs. BF16 comparison")
...
Transformer Engine (TE) is a powerful library designed to accelerate the training and inference of Transformer models on NVIDIA GPUs, particularly by leveraging 8-bit floating-point (FP8) precision on Hopper, Ada, and Blackwell GPUs. This results in superior performance and reduced memory utilization without sacrificing accuracy. TE provides optimized building blocks for popular Transformer architectures and a user-friendly API that seamlessly integrates with your preferred deep learning frameworks. A framework-agnostic C++ API is also available for broader integration with other deep learning libraries.

## Highlights

*   **FP8 Support:** Built-in support for FP8 precision on Hopper, Ada, and Blackwell GPUs.
*   **Optimized for Transformers:** Includes fused kernels and other performance optimizations.
*   **Broad Precision Support:** Optimizations extend to FP16, and BF16 on NVIDIA Ampere and later architectures.
*   **Easy Integration:** Simple modules for constructing Transformer layers, simplifying mixed-precision training.
...
For a complete, in-depth tutorial, explore our [Quickstart Notebook](https://github.com/NVIDIA/TransformerEngine/blob/main/docs/examples/quickstart.ipynb).
...
The fastest way to get started is using pre-built Docker images from the [NVIDIA GPU Cloud (NGC) Catalog](https://catalog.ngc.nvidia.com/orgs/nvidia/containers/pytorch).

To run the NGC PyTorch container interactively:

```bash
docker run --gpus all -it --rm nvcr.io/nvidia/pytorch:25.04-py3
```

For the NGC JAX container:

```bash
docker run --gpus all -it --rm nvcr.io/nvidia/jax:25.04-py3
```

(Replace `25.04` with your desired container version.)
...
Specify frameworks with the `NVTE_FRAMEWORK` environment variable:
...
[Installation from Source](https://docs.nvidia.com/deeplearning/transformer-engine/user-guide/installation.html#installation-from-source)
...
Transformer Engine supports FlashAttention-2 and FlashAttention-3 in PyTorch for enhanced performance. FlashAttention-3, introduced in v1.11, is prioritized.

Verify the FlashAttention version in use by setting these environment variables:

```bash
NVTE_DEBUG=1 NVTE_DEBUG_LEVEL=1 python your_script.py
```

FlashAttention-2 compilation may be resource-intensive and can lead to out-of-memory errors. Resolve this by setting:  `MAX_JOBS=1`.
...
1.  **ABI Compatibility Issues:**
    ...
    *   **Solution:** Ensure PyTorch and Transformer Engine are built with the same C++ ABI. Rebuild PyTorch from source, if needed.
    *   **Context:** Errors can occur if PyTorch is built with a different C++ ABI than your system's default.
...
2.  **Missing Headers or Libraries:**
    ...
    *   **Solution:** Install missing development packages or set environment variables:
        ...
        *   Set `CXX` if CMake cannot find a C++ compiler.
        *   Verify all paths.
...
The attention mask definition has been unified.  In Transformer Engine, `True` now means masking out a position and `False` means including that position in attention calculation.

Example:

```
# Input Sequence: [a, a, a, 0, 0, b, b, 0, 0, 0, c, c, c, c, 0]
# Padding Mask (v1.7 and later): [False, False, False, True, True, False, False, True, True, True, False, False, False, False, True]
```

## FP8 Convergence

Extensive testing across various models shows **no significant difference** between FP8 and BF16 training loss curves. FP8 accuracy has also been validated on LLM tasks.
...
Contributions are welcome! Refer to the `<CONTRIBUTING.rst>`_ guide for instructions.
...
*   [12/2023] [New NVIDIA NeMo Framework Features and NVIDIA H200](https://developer.nvidia.com/blog/new-nvidia-nemo-framework-features-and-nvidia-h200-supercharge-llm-training-performance-and-versatility/)
     ![H200-NeMo-performance](docs/examples/H200-NeMo-performance.png "H200")
