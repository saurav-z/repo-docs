...
Transformer Engine (TE) is your key to unlocking faster and more efficient Transformer model training and inference on NVIDIA GPUs, including FP8 support.  [Explore the original repo](https://github.com/NVIDIA/TransformerEngine) for more details.

**Key Features:**

*   🚀 **FP8 Precision:** Train and infer Transformer models with cutting-edge 8-bit floating point (FP8) precision on Hopper, Ada, and Blackwell GPUs, achieving superior performance with reduced memory usage.
*   ⚡ **Optimized Building Blocks:** Leverage highly optimized components for popular Transformer architectures, including fused kernels for enhanced speed.
*   ⚙️ **Framework Agnostic API:** Integrate TE with your existing deep learning libraries with a C++ API for seamless FP8 support.
*   🛠️ **Easy-to-Use Modules:** Build Transformer layers effortlessly with dedicated modules and integrated support for FP8.
*   🌐 **Broad Compatibility:** Supports FP16, BF16, and FP8 precision on NVIDIA Ampere, Ada, Hopper, and Blackwell architectures.
*   📦 **Integration:** Seamlessly integrates with popular LLM frameworks like DeepSpeed, Hugging Face Accelerate, and others.
...
For a comprehensive guide, check out our [Quickstart Notebook](https://github.com/NVIDIA/TransformerEngine/blob/main/docs/examples/quickstart.ipynb).
...
**Specify Framework (when installing from GitHub):**
...
**Note:** FlashAttention-2 compilation may require significant RAM.  Consider setting `MAX_JOBS=1` to prevent out-of-memory errors.
...
1.  **ABI Compatibility:**
    ...
    *   **Context:** This is common with pip-installed PyTorch.
...
The padding mask definition has changed in PyTorch to align with other frameworks. In v1.7 and later, `True` in the attention mask now means masking out the corresponding position, and `False` means including that position.
...
FP8 has demonstrated excellent convergence across various models.  Testing shows **no significant difference** in loss curves compared to BF16 training.  Accuracy has also been validated on LLM tasks.  See the table for convergence results:
