...
**Transformer Engine empowers faster and more memory-efficient Transformer model training and inference, leveraging the power of FP8 precision.** [View the original repository](https://github.com/NVIDIA/TransformerEngine)

**Key Features:**

*   **FP8 Support:** Harness the performance benefits of 8-bit floating-point (FP8) precision on NVIDIA Hopper, Ada, and Blackwell GPUs.
*   **Optimized Building Blocks:** Utilize highly optimized modules for popular Transformer architectures, boosting both speed and efficiency.
*   **Simplified Integration:** Integrate FP8 support seamlessly with your existing PyTorch and JAX code through a user-friendly API.
*   **Mixed Precision API:** Automatic mixed precision-like API that can be used seamlessly with your framework-specific code.
*   **Framework Agnostic C++ API:** Framework agnostic C++ API that can be integrated with other deep learning libraries to enable FP8
    support for Transformers.
*   **Performance across Precisions:** Optimizations for FP16, BF16, and FP8 across NVIDIA Ampere and later GPU architectures.
...
Transformer Engine (TE) is a library designed to accelerate Transformer models on NVIDIA GPUs. It leverages cutting-edge techniques like 8-bit floating-point (FP8) precision on Hopper, Ada, and Blackwell GPUs.  This leads to substantial performance gains and reduced memory usage during both training and inference.  TE provides optimized building blocks for popular Transformer architectures and a user-friendly API compatible with framework-specific code.
...
The easiest way to start with Transformer Engine is using Docker images on [NVIDIA GPU Cloud (NGC) Catalog](https://catalog.ngc.nvidia.com/orgs/nvidia/containers/pytorch).
...
FP8 has been tested extensively across different model architectures and configurations, showing **no significant difference** between FP8 and BF16 training loss curves. FP8 has also been validated for accuracy on downstream LLM tasks (e.g. LAMBADA and WikiText). Below are examples of models tested for convergence across different frameworks.
...
We welcome contributions! Follow the guidelines in the `<CONTRIBUTING.rst>` guide to contribute.
