...
**Supercharge your Transformer model training and inference on NVIDIA GPUs with the power of FP8 precision, achieving faster performance and reduced memory usage.**  [See the original repository](https://github.com/NVIDIA/TransformerEngine).
...
*   **Optimized Modules:**  Easy-to-use modules for building Transformer layers with built-in optimizations, including fused kernels.
*   **Mixed Precision:**  Seamlessly integrates with existing code, supporting FP16 and BF16 on Ampere and later architectures for enhanced performance.
*   **Framework Agnostic C++ API:** Enables integration with other deep learning libraries for FP8 support in Transformers.
*   **Broad Integrations:** Compatible with popular LLM frameworks such as DeepSpeed, Hugging Face Accelerate, and more.
...
Use pre-built Docker images for a streamlined setup:
...
Specify frameworks with environment variable:
...
To verify which FlashAttention version is being used:
...
**Troubleshooting**
See the original README for full troubleshooting:

### Common Issues and Solutions:

1.  **ABI Compatibility Issues:**
    *   Ensure PyTorch and Transformer Engine are built with the same C++ ABI setting. Rebuild PyTorch from source with matching ABI.

2.  **Missing Headers or Libraries:**
    *   Install missing development packages or set environment variables to point to correct locations:
