...
Transformer Engine empowers faster and more efficient Transformer model training and inference on NVIDIA GPUs, leveraging FP8 precision for significant performance gains.  [See the original repo](https://github.com/NVIDIA/TransformerEngine).

**Key Features:**

*   **FP8 Precision:** Optimized for NVIDIA Hopper, Ada, and Blackwell GPUs, enabling faster training and inference with lower memory usage.
*   **Ease of Use:** Simple modules and an autocasting API seamlessly integrate with existing PyTorch and JAX code.
*   **Optimized Kernels:** Includes fused kernels and other optimizations for Transformer models, boosting performance.
*   **Framework Agnostic C++ API:** Allows integration with various deep learning libraries for FP8 support.
*   **Broad Compatibility:** Supports FP16, BF16, and FP8 precision across NVIDIA GPU architectures.
*   **Integration with Popular Frameworks:**  Works with DeepSpeed, Hugging Face Accelerate, and others.
...
*   **MAX\_JOBS:** Limit parallel build jobs
*   **NVTE\_BUILD\_THREADS\_PER\_JOB:** Control threads per build job

### Compiling with FlashAttention

Transformer Engine supports FlashAttention-2 and FlashAttention-3 (v1.11+ in PyTorch). Verify with:

```bash
NVTE_DEBUG=1 NVTE_DEBUG_LEVEL=1 python your_script.py
```

If FlashAttention-2 compilation fails (OOM), try setting `MAX_JOBS=1`.
...
1.  **ABI Compatibility Issues:**
    ...
    *   **Solution:** Ensure PyTorch and Transformer Engine are built with the same C++ ABI. Rebuild PyTorch.
2.  **Missing Headers or Libraries:**

    *   **Symptom:** CMake errors about missing headers (e.g., `cudnn.h`).
    *   **Solution:** Install missing development packages, set `CUDA_PATH` and `CUDNN_PATH` environment variables, and ensure `CXX` is set if CMake can't find the C++ compiler.
3.  **Build Resource Issues:**

    *   **Symptom:** Compilation hangs or OOM errors.
    *   **Solution:** Limit parallel builds with `MAX_JOBS=1 NVTE_BUILD_THREADS_PER_JOB=1`.
...
The padding mask definition changed in v1.7.  `True` in the mask now indicates padding (exclusion), aligning with other frameworks.

## FP8 Convergence

FP8 delivers comparable convergence to BF16 training across various model architectures. See examples:
...
Transformer Engine integrates with:
...
*   Attention original paper: [https://proceedings.neurips.cc/paper/2017/file/3f5ee243547dee91fbd053c1c4a845aa-Paper.pdf](https://proceedings.neurips.cc/paper/2017/file/3f5ee243547dee91fbd053c1c4a845aa-Paper.pdf)
*   Megatron-LM tensor parallel: [https://arxiv.org/pdf/1909.08053.pdf](https://arxiv.org/pdf/1909.08053.pdf)
*   Megatron-LM sequence parallel: [https://arxiv.org/pdf/2205.05198.pdf](https://arxiv.org/pdf/2205.05198.pdf)
*   FP8 Formats for Deep Learning: [https://arxiv.org/abs/2209.05433](https://arxiv.org/abs/2209.05433)
