...
**Supercharge your Transformer model performance and reduce memory usage with NVIDIA Transformer Engine, enabling faster training and inference using cutting-edge FP8 precision.** ([Back to top](#transformer-engine))
...
Transformer Engine (TE) is a powerful library designed to accelerate the performance of Transformer models on NVIDIA GPUs. It achieves this through several key techniques, including support for 8-bit floating point (FP8) precision on Hopper, Ada, and Blackwell GPUs. This results in significant performance gains and reduced memory consumption during both training and inference. The library provides highly optimized building blocks for popular Transformer architectures and offers an automatic mixed-precision-like API, compatible with framework-specific code. A framework-agnostic C++ API is also included for integration with other deep learning libraries, enabling FP8 support for Transformers.

As Transformer models grow in complexity and parameter count (e.g., BERT, GPT, T5), their training and inference become increasingly demanding in terms of memory and computation. TE addresses this challenge by enabling mixed-precision training, which combines single-precision (FP32) with lower precision formats (e.g., FP16). Hopper, Ada, and Blackwell GPUs further enhance performance with FP8 precision, offering improved performance over FP16 without sacrificing accuracy.

TE simplifies the integration of FP8 support by providing APIs that work seamlessly with popular Large Language Model (LLM) libraries. It offers a Python API with modules to easily construct Transformer layers, as well as a framework-agnostic C++ library with structs and kernels necessary for FP8 support. TE's modules internally manage scaling factors and other FP8 training requirements, making mixed-precision training significantly easier for users.

## Key Features

*   **FP8 Support:** Accelerate training and inference with FP8 precision on NVIDIA Hopper, Ada, and Blackwell GPUs.
*   **Optimized Modules:** Build Transformer layers effortlessly with easy-to-use, pre-optimized modules.
*   **Performance Enhancements:** Leverage fused kernels and other optimizations for Transformer models.
*   **Broad Precision Support:** Benefit from optimizations across various precisions, including FP16 and BF16 on NVIDIA Ampere and later GPU architectures.
...
The quickest way to get started with Transformer Engine is by using Docker images on the [NVIDIA GPU Cloud (NGC) Catalog](https://catalog.ngc.nvidia.com/orgs/nvidia/containers/pytorch).
