...
## 🌐 Unleash the Power of LLMs: Build Custom MCP Agents with MCP-Use!

**mcp-use** is an open-source Python library that connects Large Language Models (LLMs) to tools, enabling you to build powerful, custom agents. Seamlessly integrate your favorite LLM with tools like web browsers, file operations, and more, without relying on closed-source solutions. Get started at [mcp-use.com](https://mcp-use.com/) and explore the [original repo](https://github.com/mcp-use/mcp-use).
...
*   ✅ **Easy Integration:** Connect any LLM (OpenAI, Anthropic, etc.) that supports tool calling to any MCP server.
*   💻 **Code Builder:** Use the interactive [code builder](https://mcp-use.com/builder) to explore MCP capabilities and generate starter code.
*   🌐 **HTTP Support:** Seamlessly connect to MCP servers running on specific HTTP ports.
*   ⚙️ **Dynamic Server Selection:** Let agents dynamically choose the best MCP server for the task.
*   🧩 **Multi-Server Support:** Leverage multiple MCP servers within a single agent.
*   🛡️ **Tool Access Control:** Restrict access to sensitive tools like file system and network access.
*   🛠️ **Custom Agents:** Build custom agents using the flexible LangChain adapter or by creating your own.
*   🚀 **Sandboxed Execution:** Run MCP servers in a secure, isolated environment using E2B's cloud infrastructure.
*   ⏩ **Streaming Output:** Get real-time updates on agent progress.
...
Add your API keys for the chosen provider to your `.env` file.
```bash
OPENAI_API_KEY=
ANTHROPIC_API_KEY=
```
...
```
or setting the debug flag directly in the code:
```python
import mcp_use
