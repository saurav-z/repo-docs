...
Megatron-LM is a GPU-optimized library for training transformer models, offering cutting-edge performance and scalability. Explore the power of distributed training and unlock the potential of large language models with this comprehensive toolkit.  [Explore the original repo](https://github.com/NVIDIA/Megatron-LM)
...
*   **GPU-Optimized Performance:** Leverage NVIDIA GPUs for faster training and inference.
*   **Distributed Training:** Train models with billions of parameters across multiple GPUs.
*   **Advanced Parallelism Strategies:** Utilize Data Parallelism (DP), Tensor Parallelism (TP), Pipeline Parallelism (PP), Context Parallelism (CP), and Expert Parallelism (EP).
*   **Mixed Precision Support:** Train with FP16, BF16, and FP8 for optimized memory usage and speed.
*   **Modular Architecture:** Utilize `Megatron Core` building blocks to build custom training frameworks.
*   **Integration with Ecosystem Libraries:** Compatible with Transformer Engine, NeMo, Hugging Face Accelerate, and others.
*   **Comprehensive Training Examples:** Get started quickly with pre-configured training scripts for various models.
...
**→ [Complete Installation Guide](#installation)** - Includes Docker, pip variants, source installation, and system requirements

## What's New

*   🔄 **Megatron Bridge:**  Bidirectional converter for interoperability between Hugging Face and Megatron checkpoints.
*   🗺️ **MoE Roadmap:** Comprehensive roadmap for MoE features including DeepSeek-V3, Qwen3, advanced parallelism strategies, FP8 optimizations, and Blackwell performance enhancements.
*   🚀 **GPT-OSS Implementation:** Integration of advanced features like YaRN RoPE scaling and custom activation functions into Megatron Core.
*   **Megatron MoE Model Zoo:** Best practices and optimized configurations for training DeepSeek-V3, Mixtral, and Qwen3 MoE models.
*   **Multi-Data Center Training:** Improved capabilities for LLM training across long-haul data center networks.
