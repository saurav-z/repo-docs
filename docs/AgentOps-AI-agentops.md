...
<div align="center">
  <em>**Supercharge Your AI Agents: Observability, Debugging, and Optimization**</em>
</div>
...
**AgentOps is the ultimate observability and devtool platform for AI agents, enabling you to build, evaluate, and monitor your agents from prototype to production.**  Track agent performance, identify issues, and optimize costs with ease.
...
*   **Compliance and Security:** Detect prompt injection and data exfiltration attempts.
*   **Framework Integrations:** Native integrations with popular frameworks like CrewAI, AG2 (AutoGen), Camel AI, and Langchain.

## Quick Start

Install the AgentOps Python package:
...
Initialize the AgentOps client to automatically gain powerful analytics on all your LLM calls.

1.  **Get an API Key:** Obtain your API key from the [AgentOps dashboard](https://app.agentops.ai/settings/projects).
...
All your sessions are easily viewed on the [AgentOps dashboard](https://app.agentops.ai?ref=gh).
...
Add robust observability to your agents, tools, and functions with minimal code: one line at a time.

Refer to the [AgentOps documentation](http://docs.agentops.ai) for detailed examples.
...
AgentOps seamlessly integrates with the OpenAI Agents SDKs for both Python and TypeScript, enabling you to build and monitor multi-agent systems with tools, handoffs, and guardrails.
...
Integrate AgentOps with CrewAI agents in just a couple of lines of code. Simply set the `AGENTOPS_API_KEY` environment variable to start monitoring your crews on the AgentOps dashboard.
...
Add comprehensive observability and monitoring to AG2 (formerly AutoGen) agents with two lines of code.  Set the `AGENTOPS_API_KEY` environment variable and call `agentops.init()`.
...
Track and analyze CAMEL agents with full observability. Set an `AGENTOPS_API_KEY` in your environment and initialize AgentOps.
...
AgentOps supports Langchain applications. Install Langchain as an optional dependency and use the provided handler.
...
Full support for Cohere (>=5.4.0) allows for quick and effective integration.
...
Integrate AgentOps with agents built with the Anthropic Python SDK (>=0.32.0).
...
Integrate AgentOps with applications built using LlamaIndex.
...
Monitor and analyze SwarmZero agents with full observability.
...
AI agents are often slow, costly, and unreliable without the right tools.  AgentOps provides a comprehensive solution for taking your agent from prototype to production.

*   **Comprehensive Observability:** Track agent performance, user interactions, and API usage.
*   **Real-Time Monitoring:** Get instant insights via session replays, metrics, and monitoring tools.
*   **Cost Control:** Monitor and manage LLM and API call spending.
*   **Failure Detection:** Identify and respond to agent and multi-agent issues quickly.
*   **Tool Usage Statistics:** Understand tool utilization with detailed analytics.
*   **Session-Wide Metrics:** Get a holistic view of agent sessions with comprehensive statistics.

AgentOps simplifies agent observability, testing, and monitoring.

## Star History

See how the community is growing with AgentOps:
