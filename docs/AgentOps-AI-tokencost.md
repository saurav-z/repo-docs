...
# Tokencost: Accurate LLM Cost Tracking and Token Counting for AI Applications

**Accurately estimate LLM API costs and manage token usage with Tokencost, a powerful Python library.**  [Get started now!](https://github.com/AgentOps-AI/tokencost)

## Key Features

*   **Real-time LLM Cost Tracking:** Stay updated on the latest pricing for major LLM providers.
*   **Precise Token Counting:** Accurately calculate the number of tokens in your prompts and completions before making API calls.
*   **Simplified Integration:** Easily integrate cost and token calculations into your AI agent or LLM applications with a single function call.
...
Easily determine the cost of prompts and completions for OpenAI requests:
...
Count tokens in message and string prompts:
...
Tokencost uses [Tiktoken](https://github.com/openai/tiktoken), OpenAI's official tokenizer, to count tokens accurately. Tiktoken handles tokenization of both raw strings and message formats, accounting for formatting and roles. For Anthropic models above version 3, the [Anthropic beta token counting API](https://docs.anthropic.com/claude/docs/beta-api-for-counting-tokens) is used. For older Claude models, Tiktoken with the cl100k_base encoding is used for approximation.

## Cost Table

Pricing information in USD. The price table can be found [here](pricing_table.md).
