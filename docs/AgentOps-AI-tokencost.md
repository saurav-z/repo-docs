...
# Tokencost: Calculate LLM Costs Easily

**Tokencost** is a Python library that provides clientside token counting and LLM cost estimation for your AI applications, helping you understand and manage your LLM expenses.  [Check out the original repo](https://github.com/AgentOps-AI/tokencost).

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)  [![PyPI - Version](https://img.shields.io/pypi/v/tokencost)](https://pypi.org/project/tokencost/)  [![X (formerly Twitter) Follow](https://img.shields.io/twitter/follow/AgentOpsAI)](https://x.com/agentopsai)

## Key Features

*   **Accurate Token Counting:** Precisely count tokens for prompts and completions using Tiktoken, ensuring accurate cost calculations.
*   **LLM Price Tracking:** Easily track and adapt to the latest pricing changes from major LLM providers, keeping your cost estimations up-to-date.
*   **Easy Integration:** Estimate the cost of prompts and completions with a single function call.
*   **Supports message and string prompts:** Works with both message-based and string-based prompts, providing flexible usage.
*   **Anthropic Token Counting API Support**: Utilizes the Anthropic token counting API for models above version 3, ensuring accurate token counts for Claude models.
...
Estimate the cost of prompts and completions in your OpenAI requests:
...
Quickly count tokens in your prompts:
...
Tokencost uses [Tiktoken](https://github.com/openai/tiktoken), OpenAI's official tokenizer, to tokenize strings and ChatML messages.  Tiktoken breaks text into tokens, handling both raw strings and message formats with additional tokens for formatting and roles.  For Anthropic models above version 3 (like Sonnet 3.5, Haiku 3.5, and Opus 3), it leverages the [Anthropic beta token counting API](https://docs.anthropic.com/claude/docs/beta-api-for-counting-tokens) for accurate token counts. For older Claude models, it approximates using Tiktoken with the cl100k_base encoding.
