...
**Tackle long video reasoning challenges with Long-RL, a framework that leverages reinforcement learning to achieve state-of-the-art performance on long video benchmarks!**  ([Original Repo](https://github.com/NVlabs/Long-RL))
...
*   **Breakthrough Performance:** Achieve SOTA results on video benchmarks, notably with LongVILA-R1-7B reaching **65.1% / 71.1%** accuracy on VideoMME (w/o/with subtitles).
*   **Long Video Support:** Process up to **8,192 video frames** per video with configurable FPS.
*   **Multi-Modal RL Training:** Train on various modalities including video, text, and audio, and support image and video generation models.
*   **Efficient Training Infrastructure:**  Multi-modal Reinforcement Sequence Parallelism (MR-SP) system delivers up to **2.1x speedup** on long video RL training.
*   **Comprehensive Support:** Supports various models (VILA, Qwen series), algorithms (GRPO, DAPO, Reinforce), and open-ended reward settings.
*   **Accessible Demo & Resources:**  Explore the capabilities with a Gradio demo and readily available model weights on Hugging Face.
...
*   \[2025.7.24] Gradio demo released:  (https://long-rl.hanlab.ai) with our LongVILA-R1-7B model deployed.
*   \[2025.7.24]  Model weights of **LongVILA-R1-7B** released on HuggingFace (https://huggingface.co/Efficient-Large-Model/LongVILA-R1-7B). LongVILA-R1-7B achieves **65.1% / 71.1%** on VideoMME. It supports reasoning on both **multiple-choice** and **open-ended** questions, and can also switch to non-thinking mode.
...
*   \[2025.7.18] New supported features, including *Open-ended reward*, *Cached video embeddings*, and *Chunked gathering*.
...
Long-RL introduces a full-stack framework for scaling reasoning in vision-language models (VLMs) to long videos, leveraging reinforcement learning. It addresses challenges with three key components: (1) a large-scale dataset, LongVideo-Reason, (2) a two-stage training pipeline (CoT-SFT & RL), and (3) Multi-modal Reinforcement Sequence Parallelism (MR-SP) for efficient long video RL.

**Supported Models:**

*   VILA series models on image and video.
*   Qwen-VL series models on text, image, video, and audio.
*   Image and video diffusion model RL (Stable Diffusion, Wan series).
...
*   **Open-ended reward**:  Train for open-ended QAs. Requires setting `--worker.rollout.open_ended_reward=True` and exporting your OpenAI API key (`export OPENAI_API_KEY=xxx`).
*   **Cached video embeddings**: Speeds up video RL training.  Cache embeddings using `verl/utils/cache_video_embeds_vila.py` and set `--data.cache_dir` and `--worker.actor.cached_embeds_dir` in the training script.
*   **Chunked gathering**: Addresses potential CPU OOM issues.  Set `--worker.rollout.num_chunk_seq` (8/16/32).
...
Refer to example scripts in the `examples` directory, e.g.,
...
See the [EasyR1 repo](https://github.com/hiyouga/EasyR1/tree/main?tab=readme-ov-file#how-to-run-70b-model-in-multi-node-environment) for details.
...
Contributions of any kind are welcome and acknowledged.
...
*   [EasyR1](https://github.com/hiyouga/EasyR1) for providing the foundation.
...
*   [Flow-GRPO](https://github.com/yifan123/flow_grpo) for image/video generation RL inspiration.