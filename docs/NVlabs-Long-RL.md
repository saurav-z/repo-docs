...
**Long-RL empowers vision-language models to understand and reason about long videos using a novel full-stack framework built on reinforcement learning.** Check out the [original repository](https://github.com/NVlabs/Long-RL) for more details.
...
*   **Scalable RL for Long Videos:** Train on hour-long videos using sequence parallelism on a single A100 node.
*   **Multi-Modal RL Support:** Train on models with text, video, and audio inputs (e.g., Qwen-VL series).
*   **Image/Video Generation RL:** Supports RL training for image and video generation models (e.g., Stable Diffusion).
*   **High Performance:** Achieve state-of-the-art results on video benchmarks with the LongVILA-R1-7B model.
*   **Open-Ended QA Support:** Train and evaluate models on open-ended question answering tasks.
*   **Efficient Training:** Utilize cached video embeddings and chunked gathering for faster training.
*   **Comprehensive Dataset:** Leverages the LongVideo-Reason dataset with high-quality reasoning annotations.
...
*   **\[2025.7.24]** Gradio demo (https://long-rl.hanlab.ai) and LongVILA-R1-7B model weights released on Hugging Face (https://huggingface.co/Efficient-Large-Model/LongVILA-R1-7B).
*   **\[2025.7.24]** LongVILA-R1-7B achieves **65.1% / 71.1%** on VideoMME and supports both multiple-choice and open-ended questions.
*   **\[2025.7.19]** Detailed instructions and scripts for LongVideo-Reason dataset generation released.
*   **\[2025.7.18]** New features released, including Open-ended reward, Cached video embeddings, and Chunked gathering.
*   **\[2025.7.10]** Paper ([https://arxiv.org/abs/2507.07966](https://arxiv.org/abs/2507.07966)) and GitHub repo ([https://github.com/NVlabs/Long-RL](https://github.com/NVlabs/Long-RL)) released.
...
Long-RL is a full-stack framework designed to scale reasoning in vision-language models (VLMs) to long videos using reinforcement learning. It leverages:

*   A large-scale dataset, LongVideo-Reason (104K long video QA pairs)
*   A two-stage training pipeline (CoT-SFT + RL)
*   A training infrastructure for long video RL, Multi-modal Reinforcement Sequence Parallelism (MR-SP)
...
*   **Open-ended reward:** Training for open-ended QAs (non-multi-choices QAs).
*   **Cached video embeddings:** Using cached video embeddings for video RL training.
*   **Chunked gathering:** Chunked gathering for `all_gather_data_proto`.
...
Please consider citing our paper and framework if they are helpful in your research.
