...
## Oumi: Your Complete Toolkit for Building and Deploying Foundation Models

Oumi is a fully open-source platform that streamlines the entire lifecycle of foundation models, making it easy to train, evaluate, and deploy cutting-edge AI models; **[Explore Oumi on GitHub](https://github.com/oumi-ai/oumi)**.

**Key Features:**

*   **Training and Fine-tuning:** Train and fine-tune models from 10M to 405B parameters using SFT, LoRA, QLoRA, and more.
*   **Model Support:** Works with a wide variety of text and multimodal models like Llama, DeepSeek, Qwen, and Phi.
...
*   **Comprehensive Evaluation:** Evaluate models with standard benchmarks.
*   **Flexible Deployment:**  Run on laptops, clusters, and clouds (AWS, Azure, GCP, Lambda, and more).
*   **Integration:** Integrate with both open models and commercial APIs (OpenAI, Anthropic, etc.).
...
Jump in with these helpful resources:
...
Oumi provides a user-friendly CLI for common tasks:

### Oumi CLI

Use the `oumi` command to train, evaluate, and infer models with existing recipes:
...
**Note:** Oumi is in <ins>beta</ins> and under active development. While core features are stable, some advanced features may evolve.

## Why Use Oumi?

Oumi is the ideal platform for training, evaluating, and deploying your models.

Here are some key advantages:

*   🔧 **Zero Boilerplate:** Get started quickly with pre-built recipes for common models and workflows.
*   🏢 **Enterprise-Grade:** Built and validated for large-scale model training.
*   🎯 **Research-Ready:** Easy experiment reproducibility and flexible customization.
*   🌐 **Broad Model Support:** Compatible with a wide range of models, from small to very large, text-only to multimodal.
*   🚀 **SOTA Performance:**  Supports distributed training techniques (FSDP, DDP) and optimized inference engines (vLLM, SGLang).
*   🤝 **Community-Driven:**  100% open source with an active community, avoiding vendor lock-in.

## Examples & Recipes

Explore the growing collection of configurations for popular models and training workflows:

**Note:** Find a comprehensive list of supported [models](https://oumi.ai/docs/en/latest/resources/models/supported_models.html) and datasets ([SFT](https://oumi.ai/docs/en/latest/resources/datasets/sft_datasets.html), [Pre-training](https://oumi.ai/docs/en/latest/resources/datasets/pretraining_datasets.html), [Preference Tuning](https://oumi.ai/docs/en/latest/resources/datasets/preference_datasets.html), and [Vision-Language Finetuning](https://oumi.ai/docs/en/latest/resources/datasets/vl_sft_datasets.html)) in the Oumi documentation.
...
Oumi seamlessly integrates with [🤗 Transformers](https://github.com/huggingface/transformers), offering compatibility with a vast range of models.

Models marked with ✅ have been validated by the Oumi community with ready-to-use recipes.
