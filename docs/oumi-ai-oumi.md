...
**Oumi is the open-source platform empowering you to build, train, evaluate, and deploy state-of-the-art foundation models with ease.**

## Key Features

*   **Simplified Development:** Train and fine-tune models from 10M to 405B parameters using state-of-the-art techniques (SFT, LoRA, QLoRA, DPO, and more).
...
*   **Community-Driven:** Benefit from a 100% open-source platform and active community.
...
Get hands-on with Oumi using our interactive Colab notebooks:
...
Use the `oumi` command to streamline training, evaluation, and inference using pre-built [recipes](/configs/recipes):
...
Run jobs remotely on cloud platforms (AWS, Azure, GCP, Lambda, etc.) with the `oumi launch` command:
...
**Note:** Oumi is in beta and under active development. While core features are stable, some advanced features may change.

## Why Choose Oumi?

Oumi offers a complete solution for training, evaluating, and deploying models, setting it apart with:
...
| OLMo 2 Instruct           | 7B                | [Paper](https://arxiv.org/abs/2402.00838) | [Hub](https://huggingface.co/allenai/OLMo-2-1124-7