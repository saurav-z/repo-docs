...
Oumi empowers you to build state-of-the-art foundation models with a complete, open-source platform, streamlining every stage from data preparation to deployment.

### Key Features

*   **Comprehensive Lifecycle:** Manage your foundation model journey from data to deployment with ease.
*   **Train & Fine-Tune:**  Train and fine-tune models from 10M to 405B parameters using state-of-the-art techniques (SFT, LoRA, QLoRA, DPO, and more).
...
*   **Evaluation & Benchmarks:** Evaluate models comprehensively across standard benchmarks.
*   **Run Anywhere:** Run on laptops, clusters, and clouds (AWS, Azure, GCP, Lambda, and more).
*   **Integrations:** Integrate with both open models and commercial APIs (OpenAI, Anthropic, Vertex AI, Together, Parasail, ...).
*   **Open Source & Community Driven:** Leverage a production-grade platform with one consistent API and the flexibility you need for research, backed by an active community.

[**Explore the Oumi Repository on GitHub**](https://github.com/oumi-ai/oumi)
...
Get up and running with Oumi using these interactive Colab notebooks:
...
Install Oumi using pip, with options for GPU support:
...
For detailed installation instructions, see the [installation guide](https://oumi.ai/docs/en/latest/get_started/installation.html).

### Oumi CLI

Use the `oumi` command-line interface for quick training, evaluation, and inference using pre-built recipes:
...
Run jobs on cloud platforms (AWS, Azure, GCP, Lambda, etc.) using the `oumi launch` command:
...
Oumi is the premier choice for building, training, evaluating and deploying foundation models, and offers several advantages:

*   **Zero Boilerplate:** Get started quickly with pre-built recipes and streamlined workflows.
*   **Enterprise-Grade:**  Built and validated by teams training models at scale.
*   **Research-Ready:** Reproducible experiments and flexible interfaces for customization.
*   **Broad Model Support:** Works with diverse model architectures, from small to very large.
*   **SOTA Performance:** Native support for distributed training (FSDP, DDP) and optimized inference (vLLM, SGLang).
*   **Community-First:**  Open source with an active community, fostering collaboration and avoiding vendor lock-in.

### Examples & Recipes

Browse the growing collection of ready-to-use configurations:
...
| Command R | 35B/104B | [Blog](https://cohere.com/blog/command-r7b) | [Hub](https://huggingface.co/CohereForAI/c4ai-command