...
**Effortlessly build and test Python wheels for macOS, Linux, Windows, and more with cibuildwheel, simplifying cross-platform compatibility.**  [Explore the cibuildwheel repository](https://github.com/pypa/cibuildwheel)
...
*   **Cross-Platform Builds:** Create wheels for macOS (Intel & Apple Silicon), Linux (manylinux & musllinux), Windows (64-bit, 32-bit, and Arm64), and Android, iOS and Pyodide.
...
*   **Flexibility:** Configure builds via environment variables, `pyproject.toml`, or command-line arguments.
...
`cibuildwheel` is designed to run within your CI/CD service. Supported platforms depend on your chosen service:
...
Here's how to configure `cibuildwheel` in a `.github/workflows/wheels.yml` file to build wheels for multiple platforms using GitHub Actions:
...
Refer to the [cibuildwheel documentation](https://cibuildwheel.pypa.io) for detailed usage, including PyPI deployment and other CI service examples.

## How It Works

[View the interactive diagram in the documentation](https://cibuildwheel.pypa.io/en/stable/#how-it-works) to understand the cibuildwheel build process:

![](docs/data/how-it-works.png)

## Configuration Options

cibuildwheel offers a range of options to customize your builds.  These can be specified in a `pyproject.toml` file, or as environment variables. See the [configuration docs](https://cibuildwheel.pypa.io/en/latest/configuration/) for details.
...
Explore these projects to see `cibuildwheel` in action:
...
> ℹ️  See more [working examples](https://cibuildwheel.pypa.io/en/stable/working-examples) in the documentation.

## Legal Note

`cibuildwheel` uses `delocate` or `auditwheel` for wheel repair, which might bundle dynamically linked libraries, potentially impacting licensing. Review licenses for any included code.
...
Learn how to contribute to `cibuildwheel` in the [contributing guidelines](https://cibuildwheel.pypa.io/en/latest/contributing/).
...
*   **Platform Maintainers:** (See the full list in the original README)
...
*   [PyO3/maturin-action](https://github.com/PyO3/maturin-action): A tool for building Rust wheels and cross-compiling.