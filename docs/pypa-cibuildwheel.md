...
**Simplify your Python package distribution!** cibuildwheel automates the building and testing of your Python wheels across macOS, Linux, Windows, and multiple Python versions, ensuring compatibility and saving you valuable time.  [Check out the original repo](https://github.com/pypa/cibuildwheel).
...
*   **Cross-Platform Compatibility:** Build wheels for macOS, Linux (including manylinux and musllinux), Windows, and iOS.
*   **Multiple Python Versions:** Supports a wide range of CPython, PyPy, and GraalPy versions (see the table below).
...
*   **Automated Testing:** Runs your library's tests within the wheel environment to ensure everything works as expected.
...
`cibuildwheel` is designed to run within your CI service.
...
Here's a basic example of how to use `cibuildwheel` in your `.github/workflows/wheels.yml` file:
...
For more detailed information, including PyPI deployment and usage with other CI services, consult the [documentation](https://cibuildwheel.pypa.io) and the [examples](https://github.com/pypa/cibuildwheel/tree/main/examples).

## How it Works

[Diagram of the cibuildwheel workflow is available in the documentation](https://cibuildwheel.pypa.io/en/stable/#how-it-works)
...
Here are some popular projects using `cibuildwheel`:
...
>   ℹ️ Explore more real-world examples on the [Working Examples](https://cibuildwheel.pypa.io/en/stable/working-examples) page in the docs.

## Legal Note

`cibuildwheel` uses `delocate` or `auditwheel` to repair wheels, potentially bundling dynamically linked libraries. This may have license implications; ensure your use complies with all applicable licenses.
...
ℹ️  For a comprehensive history, check the [changelog page in the docs](https://cibuildwheel.pypa.io/en/stable/changelog/).

---

## Contributing

For information on contributing to cibuildwheel, see the [docs](https://cibuildwheel.pypa.io/en/latest/contributing/). All contributors must adhere to the [PSF Code of Conduct](https://github.com/pypa/.github/blob/main/CODE_OF_CONDUCT.md).
...
Consider [matthew-brett/multibuild](http://github.com/matthew-brett/multibuild), a shell script toolbox for building wheels. For Rust projects, explore [maturin-action](https://github.com/PyO3/maturin-action) for optimized Rust wheel building and cross-compilation.