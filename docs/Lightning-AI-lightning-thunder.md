...
⚡ **Lightning Thunder** is a source-to-source compiler designed to accelerate your PyTorch models, making them faster, more efficient, and easier to optimize. Explore the power of Thunder on its [GitHub repository](https://github.com/Lightning-AI/lightning-thunder)!
...
*   **Blazing Fast Performance:** Achieve significant speedups (up to 40% faster) for your PyTorch models out-of-the-box.
*   **Optimized for Modern Hardware:** Leverage the latest hardware advancements, including NVIDIA Blackwell, with built-in support.
*   **Model Optimization Made Easy:** Enhance your models with custom kernels, fusions, quantization, distributed strategies, and more.
*   **FP8 & FP4 Precision Support:** Utilize FP8 and FP4 precision for increased performance and reduced memory footprint.
*   **Comprehensive Distributed Training:** Supports DistributedDataParallel (DDP), Fully Sharded Data Parallel (FSDP), and Tensor Parallelism (TP) to scale up your training.
*   **User-Friendly for All:** Simple plugins for immediate speedups for end users, and a composable framework for performance experts.
*   **CUDA Graphs Integration:** Reduce CPU overhead with CUDA Graphs.
*   **Extensible with Custom Kernels:** Easily integrate custom Triton kernels for specialized operations.
*   **Flexible and Composable:** Easily combine various optimization techniques.
...
Optimize your PyTorch modules in a few lines of code:
...
Thunder plugins provide a modular way to apply optimizations to your models.  Easily integrate distributed strategies, numerical precision improvements, memory saving techniques and more.

Examples include:

*   Scale up with DDP, FSDP, TP
*   Optimize numerical precision with FP8, MXFP8
*   Save memory with quantization
*   Reduce latency with CUDAGraphs
*   Debugging and profiling

To utilize CUDAGraphs, for example, use:
...
Thunder accelerates your PyTorch models through three key stages:

1.  **Acquisition:** Interprets Python bytecode to generate a straight-line Python program.
2.  **Transformation:** Optimizes the computation trace by applying transformations such as distribution and precision adjustments.
3.  **Execution:** Routes the optimized trace for execution through various methods: fusion (NVFuser, torch.compile), specialized libraries (cuDNN SDPA, TransformerEngine), custom kernels (Triton, CUDA), and PyTorch eager operations.
...
Experience the performance gains of Lightning Thunder.
