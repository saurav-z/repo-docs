...
<p align="center">
  <em>Build powerful, composable AI agents with ease using Model Context Protocol (MCP).</em>
</p>
...
**`mcp-agent` is the only AI framework purpose-built for the Model Context Protocol (MCP), empowering you to build robust, controllable AI agents that seamlessly integrate with MCP-aware services, enabling rapid development and deployment.** It’s a lightweight agent pattern library, making it easy to build complex AI applications.

### Key Features

*   **Simplified MCP Integration:** Handles the complexities of MCP server connections, allowing you to focus on agent logic.
*   **Composable Workflows:** Implements and extends patterns from Anthropic's "Building Effective Agents" in a modular, chainable way.
*   **Model Agnostic:**  Works with various LLMs.
*   **Multi-Agent Orchestration:** Includes an implementation of OpenAI's Swarm pattern, enabling sophisticated multi-agent workflows.
*   **Human-in-the-Loop Support:**  Allows agents to request and incorporate human input.
