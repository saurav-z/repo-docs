...
[Liger Kernel](https://github.com/linkedin/Liger-Kernel) empowers efficient Large Language Model (LLM) training by providing highly optimized Triton kernels.
...
[![Build Status AMD](https://github.com/linkedin/Liger-Kernel/actions/workflows/amd-ci.yml/badge.svg?branch=main)](https://github.com/linkedin/Liger-Kernel/actions/workflows/amd-ci.yml)
[![Build Status INTEL](https://github.com/linkedin/Liger-Kernel/actions/workflows/intel-ci.yml/badge.svg?branch=main)](https://github.com/linkedin/Liger-Kernel/actions/workflows/intel-ci.yml)

<img src="https://raw.githubusercontent.com/linkedin/Liger-Kernel/main/docs/images/logo-banner.png" alt="Liger Kernel Logo">

**Key Features:**

*   **Performance Boost:** Increase LLM training throughput by up to 20%.
*   **Memory Savings:** Reduce memory usage by up to 60%, and up to 80% for post-training tasks.
*   **Hugging Face Compatibility:** Seamless integration with Hugging Face models using patching APIs.
*   **Optimized Kernels:** Includes optimized kernels for `RMSNorm`, `RoPE`, `SwiGLU`, `CrossEntropy`, `FusedLinearCrossEntropy`, and more.
*   **Exact Computation:** Provides exact results without approximation.
*   **Minimal Dependencies:** Lightweight with minimal dependencies, requiring only Torch and Triton.
*   **Multi-GPU Support:** Compatible with various multi-GPU setups (FSDP, DeepSpeed, etc.).
*   **Post-Training Optimization:** Optimized kernels for DPO, ORPO, CPO, SimPO, KTO, JSD, and more, reducing memory usage by up to 80%.
*   **Integration with Popular Frameworks**: Compatible with Axolotl, LLaMa-Factory, SFTTrainer, Hugging Face Trainer, SWIFT, oumi.
...
Liger Kernel provides an easy way to boost LLM training.  With one line of code, you can increase throughput by more than 20% and reduce memory usage by 60%, enabling longer context lengths, larger batch sizes, and massive vocabularies.
...
Optimized post-training kernels, such as DPO, ORPO, and SimPO, help reduce memory usage by up to 80%.
...
*   [Hugging Face Trainer](https://github.com/linkedin/Liger-Kernel/tree/main/examples/huggingface): Train LLaMA 3-8B faster with memory reduction using 4 A100s with FSDP.
*   [Lightning Trainer](https://github.com/linkedin/Liger-Kernel/tree/main/examples/lightning): Increase throughput and reduce memory usage with LLaMA3-8B on MMLU dataset using 8 A100s with DeepSpeed ZeRO3.
...
# Automatically monkey-patches the model with optimized Liger kernels.
...
# Monkey-patches the model with optimized Liger kernels
...
*   `Fused Linear` kernels reduce memory usage by up to 80%, ideal for HBM-constrained workloads.
*   Other kernels utilize fusion and in-place techniques for performance optimization.
