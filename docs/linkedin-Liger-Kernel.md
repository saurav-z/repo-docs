...
**Liger Kernel accelerates Large Language Model (LLM) training by up to 20% and reduces memory usage by 60% using efficient Triton kernels.**  [Go to the original repository](https://github.com/linkedin/Liger-Kernel)
...
**Quick Links:** [Installation](#installation) | [Getting Started](#getting-started) | [Examples](#examples) | [High-level APIs](#high-level-apis) | [Low-level APIs](#low-level-apis) | [Cite this Work](#cite-this-work)
...
*   **Memory Savings:** Reduce memory usage by up to 60%, enabling larger batch sizes, longer context lengths, and massive vocabularies.
*   **Hugging Face Compatibility:** Seamlessly integrate with Hugging Face models (RMSNorm, RoPE, SwiGLU, CrossEntropy, FusedLinearCrossEntropy, and more).
*   **Post-Training Optimization:** Optimized kernels for DPO, ORPO, CPO, SimPO, KTO, JSD, and other alignment/distillation tasks reduce memory usage by up to 80%.
*   **Exact Computation:**  No approximations; rigorous unit tests and convergence testing ensure accuracy.
*   **Lightweight:** Minimal dependencies (Torch and Triton only).
*   **Multi-GPU Support:** Compatible with PyTorch FSDP, DeepSpeed, DDP, and other multi-GPU setups.
*   **Integration Ready:** Works out of the box with Flash Attention, PyTorch FSDP, and Microsoft DeepSpeed, plus trainer framework support like Axolotl, LLaMa-Factory, SFTTrainer, and more.
...
<details>
  ...
  *   **2024/12/11:** v0.5.0 released with up to 80% memory efficiency for post-training loss (DPO, ORPO, CPO, etc.).
  *   **2024/12/5:** Released LinkedIn Engineering Blog: [Liger-Kernel: Empowering an open source ecosystem of Triton Kernels for Efficient LLM Training](https://www.linkedin.com/blog/engineering/open-source/liger-kernel-open-source-ecosystem-for-efficient-llm-training)
  *   **2024/11/6:** v0.4.0 release: Full AMD support, Tech Report, Modal CI, Llama-3.2-Vision!
  *   **2024/10/21:** Tech report of Liger Kernel released on Arxiv: https://arxiv.org/pdf/2410.10989
  *   **2024/9/6:** v0.2.1 released ([X post](https://x.com/liger_kernel/status/1832168197002510649)).
...
Liger Kernel's optimized kernels lead to significant improvements in training speed and memory efficiency.

| Feature        | Description                                                                      |
|----------------|----------------------------------------------------------------------------------|
| Speed Up      | Training throughput increase by up to 20%                                                    |
| Memory Reduction | Memory usage reduction by up to 60%                                                 |

![Speed Up](https://raw.githubusercontent.com/linkedin/Liger-Kernel/main/docs/images/e2e-tps.png)
...
Take advantage of optimized post-training kernels (like DPO, ORPO, SimPO) for up to 80% memory savings. These kernels are available as Python modules.
...
Explore practical use cases and implementation details in our example scripts.
...
*   `torch >= 2.5.0` (Install according to the PyTorch official webpage instructions)
...
*   `transformers >= 4.x`: Required for patching transformers models. The specific model you are using will dictate the minimum transformers version.
...
The simplest method; the modeling code will be automatically patched if the model type is supported.
...
Apply optimized Liger Kernels to Hugging Face models using patching APIs.
...
Build your models using individual kernels.
...
*   **Fused Linear Kernels:** Reduce memory usage by up to 80% by combining linear layers with loss functions, ideal for HBM-constrained workloads.
...
*   **Collaboration:** Send an email to Yanning Chen (yannchen@linkedin.com) and Zhipeng Wang (zhipwang@linkedin.com).
