...
# Liger Kernel: Supercharge LLM Training with Efficient Triton Kernels
...
**Liger Kernel is a collection of highly optimized Triton kernels designed to dramatically accelerate Large Language Model (LLM) training, reducing memory usage and boosting throughput.** Dive into the [original repository](https://github.com/linkedin/Liger-Kernel) for more details.
...
*   **Up to 20% Faster Training:** Accelerate your LLM training workflows with optimized Triton kernels.
*   **Reduce Memory Footprint by 60%:** Train larger models or use bigger batch sizes without running out of memory.
*   **Post-Training Optimization:** Achieve up to 80% memory savings for alignment and distillation tasks.
*   **Hugging Face Compatibility:** Easy integration with popular Hugging Face models and pipelines.
*   **Open Source and Community-Driven:** Benefit from a growing community and contribute to the development.

## Supercharge Your Model with Liger Kernel

<p align="center">
    <img src="https://raw.githubusercontent.com/linkedin/Liger-Kernel/main/docs/images/banner.GIF" alt="Liger Kernel Demo">
</p>

With a single line of code, Liger Kernel can significantly increase your training throughput and reduce memory usage, enabling longer context lengths, larger batch sizes, and massive vocabularies.
...
Liger Kernel offers optimized post-training kernels for tasks like DPO, ORPO, and SimPO, reducing memory usage by up to 80%. These kernels are easily usable as Python modules.
...
Liger Kernel offers example implementations across various use cases:
...
*   **Easy Integration:** Simply patch your Hugging Face model with one line of code, or compose your own model using our Liger Kernel modules.
*   **Performance & Memory Optimization:** Leveraging kernel fusion, in-place replacement, and chunking techniques to boost multi-GPU training throughput by 20% and reduce memory usage by 60% for layers like **RMSNorm**, **RoPE**, **SwiGLU**, and **CrossEntropy**.
*   **Accuracy Guaranteed:** Exact computation with rigorous unit tests and convergence testing against training runs without Liger Kernel.
*   **Minimal Dependencies:** Requires only Torch and Triton - no extra libraries.
*   **Multi-GPU Support:** Compatible with multi-GPU setups (PyTorch FSDP, DeepSpeed, DDP, etc.).
*   **Framework Integrations:** Compatible with [Axolotl](https://github.com/axolotl-ai-cloud/axolotl), [LLaMa-Factory](https://github.com/hiyouga/LLaMA-Factory), [SFTTrainer](https://github.com/huggingface/trl/releases/tag/v0.10.1), [Hugging Face Trainer](https://github.com/huggingface/transformers/pull/32860), [SWIFT](https://github.com/modelscope/ms-swift), [oumi](https://github.com/oumi-ai/oumi/tree/main).
...
Liger Kernel can be applied in several ways, based on your customization needs.

### 1. Use AutoLigerKernelForCausalLM

The `AutoLigerKernelForCausalLM` is the simplest approach.
...
Use the [patching APIs](#patching) to swap Hugging Face models with optimized Liger Kernels.
...
Use individual [kernels](https://github.com/linkedin/Liger-Kernel?tab=readme-ov-file#model-kernels) to create custom models.
...
*   `Fused Linear` kernels reduce memory usage by up to 80% when combining linear layers with losses.
*   Other kernels use fusion and in-place techniques to optimize performance and memory.
