...
<p align="center">
  <a href="https://github.com/linkedin/Liger-Kernel">
    <img src="https://raw.githubusercontent.com/linkedin/Liger-Kernel/main/docs/images/logo-banner.png" alt="Liger Kernel Logo" width="80%">
  </a>
</p>

**Liger Kernel empowers faster and more efficient Large Language Model (LLM) training by providing optimized Triton kernels.**

[![Downloads](https://img.shields.io/pypi/dm/liger-kernel?label=Downloads&color=blue)](https://pypi.org/project/liger-kernel)
...
*   **Memory Savings:** Reduce memory usage by up to **60%**, enabling larger batch sizes and context lengths.
*   **Hugging Face Compatibility:**  Seamlessly integrates with Hugging Face models, including  `RMSNorm`, `RoPE`, `SwiGLU`, `CrossEntropy`, and `FusedLinearCrossEntropy`, with more layers in development.
*   **Post-Training Optimization:** Optimized kernels for alignment and distillation tasks with up to **80% memory savings**. Supports DPO, CPO, ORPO, SimPO, KTO, JSD, and more.
*   **Hardware Agnostic:**  Runs efficiently on both NVIDIA (CUDA) and AMD (ROCm) GPUs.
*   **Easy to Use:**  Simple patching or composition using Liger Kernel modules.
*   **Exact Computation:**  Ensures accuracy with no approximations.
*   **Minimal Dependencies:** Requires only Torch and Triton.
*   **Multi-GPU Support:** Compatible with various multi-GPU strategies (PyTorch FSDP, DeepSpeed, DDP, etc.).
*   **Integration with Popular Frameworks:** Works with [Axolotl](https://github.com/axolotl-ai-cloud/axolotl), [LLaMa-Factory](https://github.com/hiyouga/LLaMA-Factory), [SFTTrainer](https://github.com/huggingface/trl/releases/tag/v0.10.1), [Hugging Face Trainer](https://github.com/huggingface/transformers/pull/32860), [SWIFT](https://github.com/modelscope/ms-swift), [oumi](https://github.com/oumi-ai/oumi/tree/main).

##  Supercharge Your LLM Training

<p align="center">
    <img src="https://raw.githubusercontent.com/linkedin/Liger-Kernel/main/docs/images/banner.GIF" alt="Performance Gains" width="100%">
</p>

Liger Kernel boosts LLM training performance and reduces memory footprint by utilizing optimized Triton kernels.

| Speed Up (Example)                  | Memory Reduction (Example)        |
|------------------------------------------|-----------------------------------------|
...
Liger Kernel provides optimized post-training kernels like DPO, ORPO, SimPO, and more.
...
Explore these examples to see Liger Kernel in action:
...
*   `torch >= 2.5.0` (Install according to the instruction on the Pytorch official webpage.)
...
*   `transformers >= 4.x`: Required for using the transformers models patching APIs. The specific model version determines the minimum `transformers` version.

> **Note:**
> Our kernels benefit from the broad hardware support offered by [Triton](https://github.com/triton-lang/triton).
...
Integrate Liger Kernel into your projects using these methods:

### 1. Use AutoLigerKernelForCausalLM

This is the easiest approach. If the model is supported, the code is automatically patched with default settings.
...
Use patching APIs to replace Hugging Face models with optimized Liger Kernels.

```python
import transformers
from liger_kernel.transformers import apply_liger_kernel_to_llama

# Automatically patch the model with optimized Liger kernels
apply_liger_kernel_to_llama()

# Alternatively, specify the kernels
apply_liger_kernel_to_llama(
  rope=True,
  swiglu=True,
  cross_entropy=True,
...
Build models using individual kernels.
...
*   Fused linear kernels combine linear layers with loss functions to reduce memory usage by up to 80%.
...
```bibtex
@inproceedings{hsu2025ligerkernel,
