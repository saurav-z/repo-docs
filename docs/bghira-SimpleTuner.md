...
**SimpleTuner simplifies AI model training, offering flexibility and performance for a wide range of applications.** ([View on GitHub](https://github.com/bghira/SimpleTuner))

**Key Features:**

*   **Simplified Training:** Easy-to-understand code with good default settings, minimizing the need for extensive tinkering.
*   **Versatile Support:** Handles diverse image/video quantities and aspect ratios, from small datasets to large-scale collections.
*   **Cutting-Edge Models:** Supports a variety of the latest diffusion models and architectures including:
    *   HiDream
    *   Flux.1
    *   Wan 2.1 Video
    *   LTX Video
...
*   **Multi-GPU Training:** Supports training on multiple GPUs for faster results.
*   **Memory Optimization:** Features like cached embeddings and DeepSpeed integration allow training on lower VRAM GPUs.
*   **Advanced Techniques:** Includes support for techniques like LoRA, ControlNet, EMA, and Mixture of Experts, to improve model quality and training stability.
*   **Integration:** Offers integration with Hugging Face Hub, S3-compatible storage, and webhook support for notifications.

**Dive deeper with the following sections:**
...
*   **Simplicity:** Designed with sensible default settings to reduce the need for configuration.
*   **Versatility:** Built to handle various datasets, from small projects to large-scale training runs.
*   **Cutting-Edge:** Incorporates features that have proven efficacy.

## Tutorial

Begin your journey by exploring the [tutorial](/TUTORIAL.md) to understand the essential information.

For a quick start, refer to the [Quick Start](/documentation/QUICKSTART.md) guide.

For memory-constrained systems, learn about using 🤗Accelerate to configure Microsoft's DeepSpeed in the [DeepSpeed document](/documentation/DEEPSPEED.md).
...
Find detailed information about the associated toolkit in [the toolkit documentation](/toolkit/README.md).

## Setup

Follow the instructions in the [installation documentation](/INSTALL.md) for setup.

## Troubleshooting

To enable detailed logging, add `export SIMPLETUNER_LOG_LEVEL=DEBUG` to your environment.

Analyze training loop performance by setting `SIMPLETUNER_TRAINING_LOOP_LOG_LEVEL=DEBUG`.

For all available options, consult [this documentation](/OPTIONS.md).