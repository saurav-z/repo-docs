...
**Hunyuan-MT unlocks high-quality, multilingual translation with models that excel in diverse language pairs.**  Find the original repo [here](https://github.com/Tencent-Hunyuan/Hunyuan-MT).
...
*   **Leading Performance:** Achieved top rankings in the WMT25 competition, securing first place in numerous language categories.
*   **High-Quality Translation:** Offers superior translation quality, including the first open-source ensemble model (Hunyuan-MT-Chimera).
*   **Comprehensive Framework:** Utilizes a cutting-edge training framework, resulting in SOTA results.
*   **Wide Language Support:**  Supports mutual translation among 33 languages, including Chinese and five ethnic minority languages.
*   **Open Source and Accessible:** Model weights available on Hugging Face and ModelScope for easy access and deployment.

## Model Overview

Hunyuan-MT comprises the Hunyuan-MT-7B translation model and the Hunyuan-MT-Chimera ensemble model. The translation model focuses on translating text between languages, while the ensemble model combines multiple translations for enhanced quality.
...
| Hunyuan-MT-Chimera-fp8     | Hunyuan 7B translation ensemble model, FP8 quantization | 🤗 [Model](https://huggingface.co/tencent/Hunyuan-MT-Chimera-7B-fp8)        |
...
This section describes how to fine-tune the Hunyuan model using `LLaMA-Factory`.
...
*   **Transformers Library**: Use the companion branch (Hunyuan-submitted code is pending review).
    ```bash
...
2.  Define your dataset in the data/dataset\_info.json file using the following format:
    ```json
    "dataset_name": {
...
3.  Execute training commands:
    *   Single-node training
    ...
    ```
    *   Multi-node training
    Execute the following command on each node. Configure NNODES, NODE\_RANK, MASTER\_ADDR, and MASTER\_PORT according to your environment:
    ```bash
    export DISABLE_VERSION_CHECK=1
...
The Hunyuan-MT models are optimized for efficient deployment via quantization.

### FP8 Quantization

Utilizing [AngelSlim](https://github.com/tencent/AngelSlim), FP8 static quantization is used to improve inference efficiency and reduce the deployment threshold. You can use AngelSlim or directly download our quantization completed open source model to use [AngelSlim](https://huggingface.co/AngelSlim).

## Deployment

Deploy your Hunyuan-MT models using frameworks like **TensorRT-LLM**, **vLLM**, or **SGLang**.
...
If you use Hunyuan-MT in your research, please cite it as follows:
...
For inquiries, please contact the open-source team or reach out via email: hunyuan_opensource@tencent.com.