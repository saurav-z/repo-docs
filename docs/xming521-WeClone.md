...
**Want a digital avatar that embodies your unique personality?** WeClone offers a comprehensive, user-friendly solution to create a digital twin from your chat history, allowing you to fine-tune LLMs and deploy them on various platforms. ([View Original Repo](https://github.com/xming521/WeClone))
...
*   **End-to-End Solution:** From chat data extraction and preprocessing to model training and deployment, WeClone provides a complete workflow.
*   **Fine-tuning with Flavor:** Fine-tune LLMs using your chat history, including image data, to capture your unique style and personality.
*   **Platform Integration:** Currently integrates with Telegram. WhatsApp and more platforms coming soon.
*   **Privacy-Focused:** Includes privacy information filtering options with localized fine-tuning and deployment for secure and controllable data.
...
> [!IMPORTANT]
>
> *   WeClone is in rapid iteration; current performance doesn't reflect final results.
> *   LLM fine-tuning effectiveness relies on model size, data quantity, and quality.  Larger models and more data generally yield better results.
> *   7B models can be less effective, 14B models show some ability, while 32B+ models perform much better.
> *   Windows environment testing is limited; WSL is recommended.
...
Test suitable temperature and top\_p values, then modify `infer_args` in settings.jsonc for subsequent inference use.
