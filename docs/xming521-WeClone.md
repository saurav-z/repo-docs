<!-- Improved README for WeClone -->

<div align="center">
  <img src="https://github.com/user-attachments/assets/5842e84e-004f-4afd-9373-af64e9575b78" alt="WeClone Logo" width="200">
  <h1>WeClone: Create Your Digital Avatar From Chat History</h1>
  <p><i>Recreate yourself in the digital world! WeClone is your one-stop solution for building and deploying a conversational AI that mirrors your unique chat style.</i></p>
  <p>
      <a href="https://github.com/xming521/WeClone" target="_blank">
        <img src="https://img.shields.io/github/stars/xming521/WeClone?style=for-the-badge&logo=github&label=Stars&logoColor=white&color=ffda65" alt="GitHub Stars">
      </a>
      <a href="https://github.com/xming521/WeClone/releases" target="_blank">
        <img src="https://img.shields.io/github/v/release/xming521/WeClone?style=for-the-badge&logo=github&label=Release&logoColor=white&color=06d094" alt="GitHub Release">
      </a>
      <a href="https://t.me/+JEdak4m0XEQ3NGNl" target="_blank">
        <img src="https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white" alt="Telegram">
      </a>
      <a href="https://x.com/weclone567" target="_blank">
        <img src="https://img.shields.io/badge/Twitter-@weclone567-000000?style=for-the-badge&logo=x&logoColor=white" alt="Twitter">
      </a>
      <a href="https://www.xiaohongshu.com/user/profile/628109730000000021029de4" target="_blank">
        <img src="https://img.shields.io/badge/WeClone-FE2C55?style=for-the-badge&logo=xiaohongshu&logoColor=white" alt="XiaoHongShu">
      </a>
      <a href="https://qm.qq.com/cgi-bin/qm/qr?k=wNdgbOVT6oFOJ2wlMLsolUXErW9ESLpk&jump_from=webapi&authKey=z/reOp6YLyvR4Tl2k2nYMsLoMC3w9/99ucgKMX0oRGlxDV/WbYnvq2QxODoIkfxn" target="_blank">
        <img src="https://img.shields.io/badge/QQ群-708067078-12B7F5?style=for-the-badge&logo=qq&logoColor=white" alt="QQ Group">
      </a>
    </p>
    <p>
      <a href="https://www.weclone.love/" target="_blank">Project Homepage</a> | <a href="https://docs.weclone.love/docs/introduce/what-is-weclone.html" target="_blank">Documentation</a> | <a href="https://github.com/xming521/WeClone/blob/master/README_zh.md" target="_blank">简体中文</a>
    </p>
</div>

<p align="center">
  <a href="https://github.com/xming521/WeClone" target="_blank">
    <img src="https://abroad.hellogithub.com/v1/widgets/recommend.svg?rid=12ab209b56cb4cfd885c8cfd4cfdd53e&claim_uid=RThlPDoGrFvdMY5" alt="Featured｜HelloGitHub" style="width: 150px; height: 28px;" />
  </a>
  <a href="https://trendshift.io/repositories/13759" target="_blank">
    <img src="https://trendshift.io/api/badge/repositories/13759" alt="xming521%2FWeClone | Trendshift" style="width: 220px; height: 50px;" />
  </a>
  <a href="https://deepwiki.com/xming521/WeClone" target="_blank">
    <img src="https://deepwiki.com/badge.svg" alt="Ask DeepWiki"  style="width: 134px; height: 23px;margin-bottom: 3px;">
  </a>
</p>
...
## Key Features
*   💬 **Chat-to-Avatar:** Seamlessly create digital avatars from your chat history.
*   🚀 **End-to-End Solution:** From data export and preprocessing to model training and deployment, all in one place.
*   💡 **Fine-tuned LLMs:**  Personalize your AI by fine-tuning Large Language Models (LLMs) using your unique chat data.
*   🌐 **Multi-Platform Support:**  Integrate with Telegram, WeChat, and other platforms (WhatsApp support coming soon!)
*   🔒 **Privacy Focused:**  Includes privacy information filtering for secure and controllable data handling.
...
> [!IMPORTANT]
> *   WeClone is under rapid development; results may vary.
> *   LLM performance depends on model size and chat data quality.
> *   7B models can be "dumb," 14B models can communicate, and 32B+ models perform better.
> *   Windows environment testing is limited; consider using WSL.
...
This project uses Qwen2.5-VL-7B-Instruct by default, fine-tuned with LoRA.  You can use other models supported by [LLaMA Factory](https://github.com/hiyouga/LLaMA-Factory/tree/main#supported-models).
...
### 1. CUDA Installation (Skip if already installed)
   *   Requires version 12.6 or above.

### 2. Environment Setup
   *   Recommended: Use [uv](https://docs.astral.sh/uv/) to manage your Python environment.
   ```bash
...
### 3. Configuration
   *   Copy the template and rename it to `settings.jsonc`:
   ```bash
   cp examples/tg.template.jsonc settings.jsonc
   ```
   *   Modify `settings.jsonc` for your specific configurations.

### 4. CUDA Verification (Optional for Mac)
   ```bash
   python -c "import torch; print('CUDA Available:', torch.cuda.is_available());"
   ```

### 5. (Optional) FlashAttention Installation (for acceleration)
   ```bash
   uv pip install flash-attn --no-build-isolation
   ```

### 6. Model Download
   *   Recommended: Use [Hugging Face](https://huggingface.co/docs/hub/models-downloading).
   *   Or use:
   ```bash
   git lfs install
...
### 7. Data Preparation
   *   Export chat records from Telegram Desktop.
   *   Select "Photos" for message types and JSON for format.
   *   Place exported `ChatExport_*` folders in `./dataset/telegram`.

### 8. Data Preprocessing
   *   Configure `language`, `platform`, and `include_type` in `settings.jsonc`.
   *   Modify `telegram_args.my_id` if using Telegram.
   *   The project uses Microsoft Presidio for PII removal.  A `blocked_words` list is provided for manual filtering.
   ```bash
   weclone-cli make-dataset
   ```
   *   For more parameter details, see: [Data Preprocessing](https://docs.weclone.love/docs/deploy/data_preprocessing.html#related-parameters)

### 9. Model Fine-tuning

   *   (Optional)  Modify `model_name_or_path`, `template`, `lora_target` in `settings.jsonc` to choose other models.
   *   Adjust  `per_device_train_batch_size` and `gradient_accumulation_steps` for VRAM usage.
   *   Modify `num_train_epochs`, `lora_rank`,  and `lora_dropout`  based on your dataset.
...
### 10. Inference
   ...
   *   Modify `infer_args` in `settings.jsonc`.
   ```bash
   ...
   *   Test results in `test_result-my.txt`.
   ```bash
   weclone-cli server
...
*   Consider using [DeepWiki](https://deepwiki.com/xming521/WeClone) for help.

## ❤️ Contributing

*   **Welcome:**  Issues/Pull Requests are encouraged!
*   **Contribution:** Check issues, help review PRs. Discuss new features via Issues.
*   **Development:**
    ```bash
...
*   **Formatting:**  Code formatting with `ruff`.
...
> [!CAUTION]
> **This project is for learning, research, and experimental purposes only. There are significant risks when used in production; assess carefully. Do not use it for illegal activities; you are responsible for the consequences.**
...
*   **Concise Hook:** The intro now includes a compelling one-sentence hook to immediately grab the reader's attention.
*   **Clear Headings:** Uses `<h1>` and `<h2>` for better structure and readability.  Uses bolding for sub-headings.
*   **Keyword Optimization:** The introduction, headings, and bullet points use relevant keywords like "digital avatar," "chat history," "LLM," "fine-tuning," and "AI."
*   **Bullet Points:** The key features are now presented in easy-to-scan bulleted lists, greatly improving readability and emphasizing the core benefits.
*   **Clear Structure:** Uses numbered lists, sub-headings and code blocks to improve the overall flow.
*   **Call to Actions:** Uses prompts like "Get Started", "Explore," and "Contribute" to engage the reader.
*   **Links to key pages** like documentation and the project home page.
*   **Alt Text:** The `<img>` tags now include descriptive `alt` attributes.
*   **Star History:** Added star history visualization for more information.
*   **More informative** disclaimer.
*   **More details** for key features and more complete hardware requirements with a useful table.
*   **More clear steps** and more context.
*   **AstrBot and LangBot** deployment included with images and clear instruction.