# MonkeyOCR: Revolutionizing Document Parsing with AI (Structure-Recognition-Relation Triplet Paradigm)

**MonkeyOCR** is a powerful document parsing system that leverages an innovative Structure-Recognition-Relation (SRR) triplet paradigm to efficiently and accurately extract information from documents. [**Explore the original repository**](https://github.com/Yuliang-Liu/MonkeyOCR) for more details.
...
*   **Superior Performance:** MonkeyOCR-pro-1.2B outperforms existing models on multiple benchmarks, including Chinese documents and OmniDocBench.
*   **Fast & Efficient:** Experience significant speed improvements with MonkeyOCR-pro-1.2B, optimized for efficient document processing.
*   **Comprehensive Support:** Parses diverse document types, including text, formulas, and tables.
*   **Flexible Deployment:**  Supports local installation, Docker deployment, and a user-friendly Gradio demo, and Fast API service.
*   **Open Source:** Leverage the power of open-source document parsing, with weights available on Hugging Face and ModelScope.
*   **Hardware Compatibility:** Successfully tested on various GPUs like 3090, 4090, A6000, H800, A100, and even 4060 (with quantized models) and open source community contributed hardware support.

## Benchmarks and Performance

MonkeyOCR consistently achieves state-of-the-art results across various benchmarks, surpassing both closed-source and open-source models in both speed and accuracy. See the detailed results below.
...
MonkeyOCR has been successfully tested on a variety of GPUs, including 3090, 4090, A6000, H800, A100, and the 4060 (suitable for quantized models). Community contributions have expanded support to include 50-series GPUs, H200, L20, V100, 2080 Ti and npu.

## News

*   ```2025.07.10``` 🚀 Release of [MonkeyOCR-pro-1.2B](https://huggingface.co/echo840/MonkeyOCR-pro-1.2B), a faster and more accurate model.
*   ```2025.06.12``` 🚀 Model trending on [Hugging Face](https://huggingface.co/models?sort=trending).
*   ```2025.06.05``` 🚀 Release of [MonkeyOCR](https://huggingface.co/echo840/MonkeyOCR), a model for English and Chinese document parsing.
...
Choose either Hugging Face or ModelScope to download the model.
...
1.  **Processed Markdown File** (`your.md`): Parsed document content in markdown format.
2.  **Layout Results** (`your_layout.pdf`): Layout results drawn on the original PDF.
3.  **Intermediate Block Results** (`your_middle.json`): Detailed JSON information about detected blocks.
...
Access the API documentation at http://localhost:8000/docs. Consider `lmdeploy_queue` or `vllm_queue` for improved API concurrency.

## Docker Deployment

Follow the instructions to build and run MonkeyOCR in Docker:

1.  ```bash
    cd docker
    ```
2.  Ensure NVIDIA GPU support is available in Docker via `nvidia-docker2` (if not, run `bash env.sh`).
3.  Build the Docker image:
    ...
    > \[!IMPORTANT]  If you have 20/30/40 series GPU, V100, or similar, build the patched Docker image for LMDeploy compatibility:
    >
    > ```bash
...
4.  Run the Gradio demo:
    ...
    Or run an interactive development environment:
...
*   OmniDocBench results (See the README for details on the dataset)
...
<tr>
<td><strong>MonkeyOCR-pro-1.2B <a href="https://huggingface.co/echo840/MonkeyOCR-pro-1.2