# MonkeyOCR: Advanced Document Parsing with a Triplet Paradigm

**Unlock efficient and accurate document understanding with MonkeyOCR, a cutting-edge system that excels at structure recognition and relation extraction.** [View the original repository](https://github.com/Yuliang-Liu/MonkeyOCR)
...
*   **Superior Accuracy:** Achieve an average improvement of 5.1% across various document types compared to pipeline-based methods, with significant gains in formula (15.0%) and table (8.6%) parsing.
*   **Competitive Performance:** Outperforms models like Gemini 2.5 Pro and Qwen2.5 VL-72B on English documents with a 3B-parameter model.
*   **Fast Processing Speed:** Process multi-page documents at 0.84 pages per second, surpassing MinerU and Qwen2.5 VL-7B.
*   **Flexible Output Formats:** Generates markdown, layout results (PDF), and detailed JSON files for comprehensive analysis.
*   **User-Friendly Demo:**  Try out our demo with a simple drag-and-drop interface: http://vlrlabmonkey.xyz:7685
*   **Easy Deployment:** Offers quick start guides for local installation, and convenient Docker deployment (with NVIDIA GPU support) and FastAPI services.
*   **Quantization:** Supports model quantization with AWQ for further efficiency gains.
*   **Comprehensive Benchmarks:** See the competitive performance with OmniDocBench benchmarks.

## Introduction

MonkeyOCR employs a Structure-Recognition-Relation (SRR) triplet paradigm to efficiently parse documents, simplifying the traditional multi-step modular approach. It offers superior performance across English and Chinese documents, excelling in text, formula, and table recognition.

### Key improvements

1.  **Enhanced Performance:** Compared to MinerU, we achieve an average improvement of 5.1% across nine types of Chinese and English documents, including a 15.0% gain on formulas and an 8.6% gain on tables.
2.  **State-of-the-Art Results:** Compared to end-to-end models, our 3B-parameter model achieves the best average performance on English documents, outperforming models such as Gemini 2.5 Pro and Qwen2.5 VL-72B.
3.  **Optimized Speed:** For multi-page document parsing, our method reaches a processing speed of 0.84 pages per second, surpassing MinerU (0.65) and Qwen2.5 VL-7B (0.12).
...
Refer to the [installation guide](https://github.com/Yuliang-Liu/MonkeyOCR/blob/main/docs/install_cuda.md#install-with-cuda-support) to set up your environment with CUDA support.
...
Use the following commands to parse PDF or image files:
...
MonkeyOCR generates three output files:
...
3.  **Intermediate Block Results** (`your_middle.json`): A JSON file containing detailed information about all detected blocks.
...
Start the MonkeyOCR FastAPI service:
...
Quantize the model using AWQ.  Follow instructions in the [Quantization guide](docs/Quantization.md).
...
Our demo is user-friendly:

1.  Upload a PDF or image.
2.  Click “Parse (解析)” to process the document.
3.  Select a prompt and click “Test by prompt” to perform content recognition based on the selected prompt.
...
Thanks to the contributors of [MinerU](https://github.com/opendatalab/MinerU), [DocLayout-YOLO](https://github.com/opendatalab/DocLayout-YOLO), [PyMuPDF](https://github.com/pymupdf/PyMuPDF), [layoutreader](https://github.com/ppaanngggg/layoutreader), [Qwen2.5-VL](https://github.com/QwenLM/Qwen2.5-VL), [LMDeploy](https://github.com/InternLM/lmdeploy), and [InternVL3](https://github.com/OpenGVLab/InternVL) , [M6Doc](https://github.com/HCIILAB/M6Doc), [DocLayNet](https://github.com/DS4SD/DocLayNet), [CDLA](https://github.com/buptlihang/CDLA), [D4LA](https://github.com/AlibabaResearch/AdvancedLiterateMachinery), [DocGenome](https://github.com/Alpha-Innovator/DocGenome), [PubTabNet](https://github.com/ibm-aur-nlp/PubTabNet), and [UniMER-1M](https://github.com/opendatalab/UniMERNet). We appreciate the efforts of everyone in the open-source community.

## Alternative Models

Explore these models if you have specific needs:
...
The 3B model's results are presented in this technical report.  This model is intended for non-commercial use.  For inquiries about larger models, contact xbai@hust.edu.cn or ylliu@hust.edu.cn.